...
# Service root may already be compact because apply_v3_power_monitor_fix.py now
# owns that cleanup. Accept either generated form, but remove the dead duplicate
# navigation machinery in both cases.
compact_signature = 'static const char *options[] = {"Back", "Power Statistics", "Diagnostic Log"};'
if compact_signature in service and 'showOptions("V3 Service", options, 3' in service:
    print("deduplicate V3 service root: already applied by power patch")
else:
...
# Preserve legacy persisted values 0..4; SWAP_LOCKED is appended as 5.
mesh_h = replace_once(
    mesh_h,
...
# Radio safety is deliberately enforced twice: before enqueue and at startSend
# immediately before the RF transmit path. The second check protects against a
# packet that was queued just before PREPARE SWAP was pressed.
radio = replace_once(
    radio,
    '#include "main.h"\n',
...
if 'drawPair(display, left, right, textPos[3], "POWER OFF", "CHANGE ANT")' in mesh_page:
    raise SystemExit("V3 antenna safety verification failed: old power-cycle UI remains active")
