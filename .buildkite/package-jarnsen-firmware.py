...
def find_factory_app_partition(
    app: bytes, factory: bytes, partitions: list[dict[str, int | str]]
) -> dict[str, int | str]:
    candidates = [
        p
        for p in partitions
    ...
    matches = [
        p
        for p in candidates
        if factory[int(p["offset"]) : int(p["offset"]) + len(app)] == app
    ]
    if not matches:
        summary = ", ".join(
...
def build_webflasher(app: bytes, factory: bytes, partitions: list[dict[str, int | str]]) -> bytes:
    ...
    if ota0 is None or ota1 is None:
        summary = ", ".join(
            f"{p['label'] or '?'}:type=0x{int(p['type']):02x}/sub=0x{int(p['subtype']):02x}"
            f"@0x{int(p['offset']):x}+0x{int(p['size']):x}"
            for p in partitions
        )
        raise SystemExit(f"Compatible OTA0/OTA1 partitions not found: {summary}")

    app0 = int(ota0["offset"])
    app1 = int(ota1["offset"])
    size0 = int(ota0["size"])
    size1 = int(ota1["size"])
    if app0 >= app1:
    ...
    print(
        "Validated partition-derived dual-slot Web Flasher image: "
        f"{len(payload)} bytes; app0=0x{app0:x}/0x{size0:x}, app1=0x{app1:x}/0x{size1:x}"
    )
    return payload
