...
PositionModule::PositionModule()
    : ProtobufModule("position", meshtastic_PortNum_POSITION_APP,
                     &meshtastic_Position_msg),
      concurrency::OSThread("Position") {
  precision = 0;        // safe starting value
  isPromiscuous = true; // We always want to update our nodedb, even if we are
                        // sniffing on others
  nodeStatusObserver.observe(&nodeStatus->onNewStatus);

  // Seed throttle timer from persisted transmit history so we don't
  // re-broadcast immediately after reboot
  if (transmitHistory) {
    uint32_t restored = transmitHistory->getLastSentToMeshMillis(
  ...
  // Power saving trackers should clear their position on startup to avoid
  // waking up and sending a stale position
  if ((config.device.role == meshtastic_Config_DeviceConfig_Role_TRACKER ||
       config.device.role == meshtastic_Config_DeviceConfig_Role_TAK_TRACKER) &&
...
bool PositionModule::handleReceivedProtobuf(const meshtastic_MeshPacket &mp,
                                            meshtastic_Position *pptr) {
  auto p = *pptr;
  ...
  // to set fixed location, EUD-GPS location or just the time (see also issue
  // #900)
  bool isLocal = false;
  if (isFromUs(&mp)) {
    isLocal = true;
    if (config.position.fixed_position) {
      LOG_DEBUG("Ignore incoming position update from myself except for time, "
                "because position.fixed_position is true");

#ifdef T_WATCH_S3
      // Since we return early if position.fixed_position is true, set the
      // T-Watch's RTC to the time received from the client device here
      if (p.time && channels.getByIndex(mp.channel).role ==
  ...
  LOG_DEBUG("POSITION node=%08x l=%d lat=%d lon=%d msl=%d hae=%d geo=%d "
            "pdop=%d hdop=%d vdop=%d siv=%d fxq=%d fxt=%d pts=%d "
            "time=%d",
            getFrom(&mp), mp.decoded.payload.size, p.latitude_i, p.longitude_i,
            p.altitude, p.altitude_hae, p.altitude_geoidal_separation, p.PDOP,
            p.HDOP, p.VDOP, p.sats_in_view, p.fix_quality, p.fix_type,
            p.timestamp, p.time);

  if (p.time &&
      channels.getByIndex(mp.channel).role == meshtastic_Channel_Role_PRIMARY) {
    bool force = false;

#ifdef T_WATCH_S3
    // The T-Watch appears to "pause" its RTC when shut down, such that the time
    // it reads upon powering on is the same as when it was shut down. So we
    // need to force the update here, since otherwise RTC::perhapsSetRTC will
    // ignore it because it will always be an equivalent or lesser RTCQuality
    // (RTCQualityNTP or RTCQualityNet).
    force = true;
#endif
    // Set from phone RTC Quality to RTCQualityNTP since it should be
    // approximately so
...
void PositionModule::alterReceivedProtobuf(meshtastic_MeshPacket &mp,
  ...
  if (isFromUs(&mp)) {
    ...
    mp.decoded.payload.size = pb_encode_to_bytes(
        mp.decoded.payload.bytes, sizeof(mp.decoded.payload.bytes),
        &meshtastic_Position_msg, p);
  }
}
...
void PositionModule::trySetRtc(meshtastic_Position p, bool isLocal,
  ...
  if (hasQualityTimesource() && !isLocal) {
    LOG_DEBUG("Ignore time from mesh because we have a GPS, RTC, or Phone/NTP "
              "time source in the past day");
    return;
  }
  if (!isLocal &&
...
bool PositionModule::hasQualityTimesource() {
  bool setFromPhoneOrNtpToday =
      lastSetFromPhoneNtpOrGps == 0
          ? false
          : Throttle::isWithinTimespanMs(lastSetFromPhoneNtpOrGps,
                                         SEC_PER_DAY * 1000UL);
#if MESHTASTIC_EXCLUDE_GPS
  bool hasGpsOrRtc = (rtc_found.address != ScanI2C::ADDRESS_NONE.address);
#else
  bool hasGpsOrRtc =
      hasGPS() || (rtc_found.address != ScanI2C::ADDRESS_NONE.address);
#endif
...
meshtastic_MeshPacket *PositionModule::allocPositionPacket() {
  ...
  const meshtastic_NodeInfoLite *node =
      service
          ->refreshLocalMeshNode(); // should guarantee there is now a position
  ...
  meshtastic_Position p =
      meshtastic_Position_init_default; //   Start with an empty structure
  // if localPosition is totally empty, put our last saved position (lite) in
  // there
  ...
  LOG_INFO("Position packet: time=%i lat=%i lon=%i", p.time, p.latitude_i,
           p.longitude_i);

#ifndef MESHTASTIC_EXCLUDE_ATAK
  // TAK Tracker devices should send their position in a TAK packet over the
  // ATAK port
...
meshtastic_MeshPacket *PositionModule::allocAtakPli() {
  ...
  // Use TAK config for team/role if configured, otherwise use defaults
  // (Cyan/TeamMember)
  if (moduleConfig.has_tak &&
      moduleConfig.tak.team != meshtastic_Team_Unspecifed_Color) {
  ...
  if (moduleConfig.has_tak &&
      moduleConfig.tak.role != meshtastic_MemberRole_Unspecifed) {
  ...
  strncpy(takPacket.device_callsign, owner.long_name,
          sizeof(takPacket.device_callsign) - 1);
  takPacket.device_callsign[sizeof(takPacket.device_callsign) - 1] = '\0';

  // CoT uid - ATAK drops PLI entities with empty uid; derive stable
  // "!<nodenum>" id.
  ...
  size_t proto_size =
      pb_encode_to_bytes(protobuf_bytes, sizeof(protobuf_bytes),
                         &meshtastic_TAKPacketV2_msg, &takPacket);
  ...
  LOG_DEBUG("TAK V2 PLI payload: %zu bytes (1 flags + %zu protobuf)",
            mp->decoded.payload.size, proto_size);
  return mp;
}
...
void PositionModule::sendOurPosition() {
  ...
  LOG_INFO("Send pos@%x:6 to mesh (wantReplies=%d)", localPosition.timestamp,
           requestReplies);
...
void PositionModule::sendOurPosition(NodeNum dest, bool wantReplies,
  ...
  if (config.device.role == meshtastic_Config_DeviceConfig_Role_TAK ||
    ...
    if (localPosition.time != 0 && nowEpoch != 0 &&
        nowEpoch >= localPosition.time)
      age = nowEpoch - localPosition.time;
    trackerDiagLogPosition(
        "POSITION_TX", localPosition.latitude_i, localPosition.longitude_i,
  ...
  if (IS_ONE_OF(config.device.role, meshtastic_Config_DeviceConfig_Role_TRACKER,
    ...
    if (notification) {
      ...
      sprintf(notification->message,
              "Sending position and sleeping for %us interval in a moment",
...
bool PositionModule::positionUnchangedSinceLastSend(
    const meshtastic_PositionLite &selfPos, bool useConfiguredPrecision) {
  if (lastGpsLatitude == 0 && lastGpsLongitude == 0)
    return false; // no prior broadcast to compare against

  // Broadcast channel = the one sendOurPosition() would pick (first with
  // non-zero on-wire precision). Default nodes gauge movement at that on-wire
  // (public-clamped) resolution; trackers use their own configured (unclamped)
  // precision so finer moves still count.
  ...
  for (uint8_t ch = 0; ch < 8; ch++) {
    ...
    precisionBits =
        useConfiguredPrecision
            ? getPositionPrecisionForChannel(channels.getByIndex(ch))
            : getPositionPrecisionForChannel(ch);
    break;
  }

  return positionWithinPrecisionCell(selfPos.latitude_i, selfPos.longitude_i,
                                     lastGpsLatitude, lastGpsLongitude,
                                     precisionBits);
}

bool PositionModule::positionWithinPrecisionCell(int32_t aLat, int32_t aLon,
                                                 int32_t bLat, int32_t bLon,
                                                 uint32_t precision) {
  if (precision == 0 || precision >= 32)
    return false; // sharing disabled or full precision: no coarse cell to hold
                  // within

  return truncateCoordinate(aLat, precision) ==
             truncateCoordinate(bLat, precision) &&
         truncateCoordinate(aLon, precision) ==
             truncateCoordinate(bLon, precision);
}

uint32_t
PositionModule::effectiveBroadcastIntervalMs(uint32_t configuredIntervalMs,
                                             bool stationary,
                                             uint32_t stationaryFloorMs) {
...
int32_t PositionModule::runOnce() {
  ...
  if (sleepOnNextExecution == true) {
    ...
    uint32_t nightyNightMs = Default::getConfiguredOrDefaultMs(
        config.position.position_broadcast_secs);
    LOG_DEBUG("Sleep for %ims, then awaking to send position again",
              nightyNightMs);
  ...
  uint32_t intervalMs = Default::getConfiguredOrDefaultMsScaled(
      ...
      numOnlineNodes, TrafficType::POSITION);
  uint32_t msSinceLastSend = now - lastGpsSend;
  // Only send packets if the channel util. is less than 25% utilized or we're a
  // tracker with less than 40% utilized.
  ...
  bool waitingForFreshPosition = (lastGpsSend == 0) &&
                                 !config.position.fixed_position &&
                                 !nodeDB->hasLocalPositionSinceBoot();

  // Hold to the 12h floor when fixed_position (every role: pinning yourself
  // forfeits the exception) or when stationary. A real move still goes out
  // early via smart-broadcast below. Not-fixed exceptions: lost-and-found
  // broadcasts freely; trackers judge movement at their own (unclamped)
  // precision rather than the on-wire one (useConfiguredPrecision).
  ...
  if (!stationary &&
      ...
      nodeDB->hasValidPosition(node)) {
    const bool isTracker =
        IS_ONE_OF(role, meshtastic_Config_DeviceConfig_Role_TRACKER,
                  meshtastic_Config_DeviceConfig_Role_TAK_TRACKER);
    meshtastic_PositionLite selfPos;
    if (nodeDB->copyNodePosition(node->num, selfPos))
      stationary = positionUnchangedSinceLastSend(
          selfPos, /*useConfiguredPrecision=*/isTracker);
  }
  uint32_t effectiveIntervalMs = effectiveBroadcastIntervalMs(
      intervalMs, stationary,
      (uint32_t)default_position_stationary_broadcast_secs * 1000UL);
  ...
  if (lastGpsSend == 0 || msSinceLastSend >= effectiveIntervalMs) {
    ...
    const meshtastic_NodeInfoLite *node2 =
        service->refreshLocalMeshNode(); // should guarantee there is now a
                                         // position

    if (nodeDB->hasValidPosition(node2)) {
      // The minimum time (in seconds) that would pass before we are able to
      // send a new position packet.

      meshtastic_PositionLite selfPos;
      if (!nodeDB->copyNodePosition(node->num, selfPos))
        return RUNONCE_INTERVAL; // Defensive: hasValidPosition should imply
                                 // this is non-null
      ...
      if (smartPosition.hasTraveledOverThreshold &&
        ...
        LOG_DEBUG("Sent smart pos@%x:6 to mesh (distanceTraveled=%fm, "
                  "minDistanceThreshold=%im, timeElapsed=%ims, "
                  "minTimeInterval=%ims)",
                  localPosition.timestamp, smartPosition.distanceTraveled,
                  smartPosition.distanceThreshold, msSinceLastSend,
                  minimumTimeThreshold);

        // Set the current coords as our last ones, after we've compared
        // distance with current and decided to send
        lastGpsLatitude = selfPos.latitude_i;
        lastGpsLongitude = selfPos.longitude_i;
      }
    }
  ...
  return RUNONCE_INTERVAL; // to save power only wake for our callback
                           // occasionally
}
...
void PositionModule::sendLostAndFoundText() {
  ...
  char message[128];
  int written =
      snprintf(message, sizeof(message), "🚨I'm lost! Lat / Lon: %f, %f\a",
               (lastGpsLatitude * 1e-7), (lastGpsLongitude * 1e-7));
  p->decoded.portnum = meshtastic_PortNum_TEXT_MESSAGE_APP;
  p->want_ack = false;
  if (written < 0) {
    // snprintf encoding error - send an empty payload rather than uninitialized
    // bytes.
    p->decoded.payload.size = 0;
  } else {
    // Clamp to buffer capacity (snprintf returns "would-have-written" which can
    // exceed the buffer).
    const size_t msg_len =
        std::min(static_cast<size_t>(written), sizeof(message) - 1);
...
// Helper: return imprecise (truncated + centered) lat/lon as int32 using
// current precision
static inline void computeImpreciseLatLon(int32_t inLat, int32_t inLon,
                                          uint8_t precisionBits,
                                          int32_t &outLat, int32_t &outLon) {
  if (precisionBits > 0 && precisionBits < 32) {
    // Build mask for top 'precisionBits' bits of a 32-bit unsigned field
    const uint32_t mask = (precisionBits == 32)
                              ? UINT32_MAX
                              : (UINT32_MAX << (32 - precisionBits));
    // Note: latitude_i/longitude_i are stored as signed 32-bit in meshtastic
    // code but the bitmask logic used previously operated as unsigned-preserve
    // that behavior by casting to uint32_t for masking, then back to int32_t.
    ...
    // The code previously added (1 << (31 - precision)) to produce the middle
...
struct SmartPosition PositionModule::getDistanceTraveledSinceLastSend(
    meshtastic_PositionLite currentPosition) {
  const uint32_t distanceTravelThreshold = Default::getConfiguredOrDefault(
      config.position.broadcast_smart_minimum_distance, 100);

  int32_t lastLatImprecise, lastLonImprecise;
  int32_t currentLatImprecise, currentLonImprecise;

  computeImpreciseLatLon(lastGpsLatitude, lastGpsLongitude, precision,
                         lastLatImprecise, lastLonImprecise);
  computeImpreciseLatLon(currentPosition.latitude_i,
                         currentPosition.longitude_i, precision,
                         currentLatImprecise, currentLonImprecise);

  float distMeters = GeoCoord::latLongToMeter(
      lastLatImprecise * 1e-7, lastLonImprecise * 1e-7,
      currentLatImprecise * 1e-7, currentLonImprecise * 1e-7);

  float distanceTraveled = fabsf(distMeters);

  return SmartPosition{.distanceTraveled = distanceTraveled,
                       .distanceThreshold = distanceTravelThreshold,
                       .hasTraveledOverThreshold =
                           distanceTraveled >= distanceTravelThreshold};
}
...
void PositionModule::handleNewPosition() {
  ...
  if (nodeDB->hasValidPosition(node2)) {
    ...
    if (smartPosition.hasTraveledOverThreshold &&
      ...
      // Set the current coords as our last ones, after we've compared distance
      // with current and decided to send
      lastGpsLatitude = selfPos.latitude_i;
      lastGpsLongitude = selfPos.longitude_i;
    }
  }
