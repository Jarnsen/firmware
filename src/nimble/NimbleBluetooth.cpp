...
#ifdef ARCH_ESP32
// Discard NimBLE bonds left in an incompatible on-disk format. The
// ESP-IDF/NimBLE upgrade changed the length of the fixed-size bond records
// (ble_store_value_sec), so the new host rejects every old record on each boot
// ("NVS data size mismatch for obj_type 1 ...") with no auto-recovery --
// pairing stays broken until a factory reset. Wipe the bond namespace once when
// a stored record's size differs from this build's struct; a same-size store is
// left untouched, so this never loops. Adapted from
// https://github.com/h2zero/NimBLE-Arduino/issues/740
static void purgeIncompatibleBleBonds() {
  esp_err_t initErr = nvs_flash_init();
  if (initErr != ESP_OK) {
    LOG_WARN("purgeIncompatibleBleBonds: nvs_flash_init failed, err=%d",
             (int)initErr);
  ...
  // Probe the first record of each fixed-size object type (bonds are written
  // from index 1); a stored size differing from this build's struct means the
  // store predates a format change.
  size_t sz = 0;
  bool mismatch = (nvs_get_blob(handle, "our_sec_1", nullptr, &sz) == ESP_OK &&
                   sz != sizeof(struct ble_store_value_sec)) ||
                  (nvs_get_blob(handle, "peer_sec_1", nullptr, &sz) == ESP_OK &&
                   sz != sizeof(struct ble_store_value_sec)) ||
                  (nvs_get_blob(handle, "cccd_sec_1", nullptr, &sz) == ESP_OK &&
...
static int lastBatteryLevel =
    -1; // last value written to 0x2A19, to skip redundant writes/notifies
...
// Set by onDisconnect to defer (re)starting advertising to the main task. A
// stale-bond reconnect triggers a MIC failure + NimBLE host reset; re-entering
...
// NimBLE task and stall the disconnect.
...
static void clearPairingDisplay() {
  ...
  heltecV3RuntimeSetPairingDisplay(false);
#endif
#if HAS_SCREEN
  if (screen) {
    screen->endAlert();
...
class BluetoothPhoneAPI : public PhoneAPI, public concurrency::OSThread {
  /*
    ...
    **in main task**.
    ...
    NOTIFY IS BROKEN:
      - Adding BLECharacteristic::PROPERTY_NOTIFY to FromRadioCharacteristic
    appears to break things. It is NOT backwards compatible.
...
protected:
  virtual int32_t runOnce() override {
    ...
    if (pendingStartAdvertising) {
      if (checkIsConnected()) {
        pendingStartAdvertising =
            false; // a new physical connection beat us to it; nothing to do
  ...
  virtual void onNowHasData(uint32_t fromRadioNum) override {
    ...
    if (!fromNumCharacteristic) // BLE may have been torn down; never notify a
                                // freed characteristic
      return;
...
static std::atomic<JarnsenLiveCommand> jarnsenLiveCommand{
    JarnsenLiveCommand::NONE};
...
static void runJarnsenLiveCommand() {
  const JarnsenLiveCommand command =
      jarnsenLiveCommand.exchange(JarnsenLiveCommand::NONE);
  if (command == JarnsenLiveCommand::NONE || !jarnsenLiveSession ||
      !jarnsenServiceActive())
    return;
...
class JarnsenLiveDataCallback : public BLECharacteristicCallbacks {
  void onRead(BLECharacteristic *characteristic) override {
    ...
    snprintf(payload, sizeof(payload),
             ...
             heltecV3ServiceMenuActive() ? 1U : 0U,
             p.batteryValid ? (unsigned)p.batteryPercent : 255U,
             (unsigned)p.voltageMv, p.usbPowered ? 1U : 0U,
             p.charging ? 1U : 0U, (unsigned)p.learnedCapacityMah,
             (unsigned)p.remainingCapacityMah, (unsigned)p.capacityConfidence,
             (unsigned)p.remainingSecs, (unsigned)p.measuredSecs,
             (unsigned)p.listenSecs, (unsigned)p.serviceSecs,
             (unsigned)p.bleSecs, (unsigned)p.displaySecs,
             (unsigned)p.positionTxCount);
#endif
    ...
    if (memcmp(lastToRadio, val.getData(), val.getLength()) != 0) {
      if (bluetoothPhoneAPI->fromPhoneQueueSize <
        ...
        LOG_DEBUG(
            ...
            currentWriteCount, finishMillis - startMillis, val.getLength());
#endif
      } else {
        LOG_WARN("BLE onWrite(%d): Drop ToRadio packet, fromPhoneQueue full "
                 "(%u bytes)",
                 currentWriteCount, val.getLength());
      }
    } else {
...
class NimbleBluetoothFromRadioCallback : public BLECharacteristicCallbacks {
  void onRead(BLECharacteristic *pCharacteristic) override {
    ...
    if (bluetoothPhoneAPI->toPhoneQueueSize > 0) {
      ...
      // (If deinit() is tearing the stack down, skip the wait entirely and just
      // return a 0-size response below - arming the wait here could pin this
      // NimBLE task for ~20s and stall teardown.)
      ...
      // setup tasks. bleDraining lets deinit() release an in-flight wait
      // immediately.
      while (bluetoothPhoneAPI->onReadCallbackIsWaitingForData &&
             !bleDraining && tries < 4000) {
    ...
    // Count only non-empty payload reads. Empty client polling reads are
    // intentionally ignored so a background connection cannot keep the V3
    // service awake.
    if (numBytes != 0) {
      meaningfulBleTrafficCount.fetch_add(1);
      bluetoothPhoneAPI->setIntervalFromNow(0);
...
class NimbleBluetoothSecurityCallback : public BLESecurityCallbacks {
...
#if defined(_VARIANT_HELTEC_V3)
    heltecV3RuntimeSetPairingDisplay(true);
#endif
  ...
  void onAuthenticationComplete(ble_gap_conn_desc *desc) override {
    // Called on every BLE_GAP_EVENT_ENC_CHANGE, success or failure. A
    // stale-bond reconnect yields a *failed* encryption change here -- don't
...
// Reset per-session PhoneAPI and transport state. Runs from onDisconnect, and
// again from setupService() on BLE re-enable because deinit()'s bounded
// disconnect wait can expire before the disconnect event delivers this cleanup
...
private:
  ...
  void onConnect(BLEServer *pServer, struct ble_gap_conn_desc *desc) {
    ...
    LOG_INFO("BLE conn %u peer MTU %u (target %u)", connHandle,
             pServer->getPeerMTU(connHandle), kPreferredBleMtu);
  ...
  void onDisconnect(BLEServer *pServer, struct ble_gap_conn_desc *desc) {
    ...
    if (pendingStartAdvertising && bluetoothPhoneAPI) {
      bluetoothPhoneAPI->setIntervalFromNow(0);
      concurrency::mainDelay
          .interrupt(); // wake the main loop to service the restart
    }
  }
};
...
void NimbleBluetooth::stopAdvertisingForService() {
  ...
  LOG_INFO("BLE advertising parked; NimBLE stack and idle PhoneAPI kept "
           "initialized");
#else
  shutdown();
#endif
...
#ifdef ARCH_ESP32
  return bleServer != nullptr && !serviceAdvertisingSuppressed.load() &&
         ble_hs_synced() && ble_gap_adv_active();
#else
  return false;
#endif
}
...
void NimbleBluetooth::deinit() {
  ...
  // BLEDevice::deinit() deletes the BLEServer before nimble_port_stop(); doing
  // that with a live connection dispatches synthesized unsubscribe events into
  // the freed server (LoadProhibited), so disconnect cleanly first. deinit()
  // runs on the main task; the waits below are bounded. bleDraining must be set
  // before we clear the flag / disconnect, so a read arriving now bails instead
  // of re-arming the ~20s wait and re-pinning the NimBLE task through the whole
  // teardown.
  bleDraining = true;
  if (bluetoothPhoneAPI)
    bluetoothPhoneAPI->onReadCallbackIsWaitingForData =
        false; // release any in-flight onRead

  // isDeInit must stay false here, else onDisconnect early-returns without
  // clearing the handle.
  ...
  isDeInit = true;
  pendingStartAdvertising =
      false; // stack is going away; don't let runOnce retry the adv restart
  ...
  BatteryCharacteristic =
      nullptr; // freed by deinit; clear so updateBatteryLevel() won't touch it
  fromNumCharacteristic = nullptr; // freed by deinit; a late onNowHasData()
                                   // must not notify freed memory
  logRadioCharacteristic = nullptr;
  lastBatteryLevel = -1;

  // The bounded disconnect wait above can expire before onDisconnect runs,
  // leaving the PhoneAPI observer attached with a live state machine; a later
  // mesh packet would then drive onNowHasData() into the now-freed
  // characteristics. Detach the observer and reset session state
  // unconditionally.
  resetBleSessionState();
#endif
}
...
void NimbleBluetooth::setup() {
  ...
  // deinit() latches these teardown guards; clear them so a re-init on the same
  // boot (e.g. an admin disable-bluetooth followed by re-enable) doesn't leave
  // onRead stuck draining or onDisconnect early-returning without clearing the
  // connection handle.
  bleDraining = false;
  isDeInit = false;
  serviceAdvertisingSuppressed = false;
...
#ifdef ARCH_ESP32
  // Runs before BLEDevice::init() reads the bond store, but logs after the
  // "Init" line above so any bond-cleanup output doesn't appear to precede the
  // module init.
  purgeIncompatibleBleBonds(); // wipe bonds left in an incompatible on-disk
                               // format (post-upgrade)
#endif
  ...
  // Statics: setup() re-runs on BLE re-enable, and the library never frees
  // these caller-owned callback objects, so register the same instances every
  // cycle.
  ...
  if (nameRc != 0) {
    LOG_ERROR("ble_svc_gap_device_name_set: rc=%d %s", nameRc,
              BLEUtils::returnCodeToString(nameRc));
  }

  static NimbleBluetoothServerCallback serverCallbacks(
      this); // safe: NimbleBluetooth is a never-deleted singleton
...
void NimbleBluetooth::setupService() {
  ...
  if (config.bluetooth.mode ==
    ...
    ToRadioCharacteristic = bleService->createCharacteristic(
        TORADIO_UUID, BLECharacteristic::PROPERTY_WRITE);
    ...
    FromRadioCharacteristic = bleService->createCharacteristic(
        FROMRADIO_UUID, BLECharacteristic::PROPERTY_READ);
    ...
    fromNumCharacteristic = bleService->createCharacteristic(
        FROMNUM_UUID, BLECharacteristic::PROPERTY_NOTIFY |
                          BLECharacteristic::PROPERTY_READ |
                          BLECharacteristic::PROPERTY_READ_AUTHEN |
                          BLECharacteristic::PROPERTY_READ_ENC);
    logRadioCharacteristic = bleService->createCharacteristic(
        LOGRADIO_UUID, BLECharacteristic::PROPERTY_NOTIFY |
                           BLECharacteristic::PROPERTY_READ |
                           BLECharacteristic::PROPERTY_READ_AUTHEN |
                           BLECharacteristic::PROPERTY_READ_ENC);
  }
  // setupService() re-runs on BLE re-enable; a fresh BluetoothPhoneAPI here
  // would leak the old one as a still-scheduled zombie OSThread, so reuse it
  // and reset its state (a skipped onDisconnect during deinit() can leave the
  // previous session's behind).
  if (!bluetoothPhoneAPI)
    bluetoothPhoneAPI = new BluetoothPhoneAPI();
  else
    resetBleSessionState();

  // The characteristics are new each cycle, so setCallbacks() must re-run every
  // time.
...
#if defined(HELTEC_TRACKER_V1_1) || defined(_VARIANT_HELTEC_V3)
  uint32_t controlProperties = BLECharacteristic::PROPERTY_WRITE |
                               BLECharacteristic::PROPERTY_READ |
                               BLECharacteristic::PROPERTY_WRITE_ENC |
                               BLECharacteristic::PROPERTY_READ_ENC;
  uint32_t dataProperties =
      BLECharacteristic::PROPERTY_READ | BLECharacteristic::PROPERTY_READ_ENC;
  if (config.bluetooth.mode !=
      meshtastic_Config_BluetoothConfig_PairingMode_NO_PIN) {
    controlProperties |= BLECharacteristic::PROPERTY_WRITE_AUTHEN |
                         BLECharacteristic::PROPERTY_READ_AUTHEN;
  ...
  BLECharacteristic *diagData =
      bleService->createCharacteristic(JARNSEN_DIAG_DATA_UUID, dataProperties);
  BLECharacteristic *liveControl = bleService->createCharacteristic(
      JARNSEN_LIVE_CONTROL_UUID, controlProperties);
  BLECharacteristic *liveData =
      bleService->createCharacteristic(JARNSEN_LIVE_DATA_UUID, dataProperties);
  ...
  BLEService *batteryService = bleServer->createService(
      BLEUUID((uint16_t)0x180f)); // 0x180F is the Battery Service
  ...
  BatteryCharacteristic =
      batteryService
          ->createCharacteristic( // 0x2A19 is the Battery Level characteristic)
              (uint16_t)0x2a19, BLECharacteristic::PROPERTY_READ |
                                    BLECharacteristic::PROPERTY_NOTIFY);
  BatteryCharacteristic->addDescriptor(batteryLevelDescriptor);
  // Seed an initial 0-100 level so an early read of 0x2A19 returns a valid
  // value.
  uint8_t initialLevel = (powerStatus && powerStatus->getHasBattery())
...
void updateBatteryLevel(uint8_t level) {
  ...
  // Cache the value so a READ works without a subscriber; notify only when
  // connected.
