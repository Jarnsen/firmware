...
class JarnsenLiveDataCallback : public BLECharacteristicCallbacks
{
    void onRead(BLECharacteristic *characteristic) override
    {
        ...
        snprintf(payload, sizeof(payload),
                 ...
                 "%u,\"g\":%u,\"bl\":%u,"
                 "\"ds\":%u,\"tx\":%u}",
                 page, jarnsenServiceActive() ? 1U : 0U, trackerServiceMenuActive() ? 1U : 0U,
                 ...
                 (unsigned)p.capacityConfidence, (unsigned)p.remainingSecs, (unsigned)p.measuredSecs, (unsigned)p.movingSecs,
                 (unsigned)p.parkedSecs, (unsigned)p.gnssSecs, (unsigned)p.bleSecs, (unsigned)p.displaySecs,
...
void NimbleBluetooth::suspend()
{
    ...
    if (connHandle != BLE_HS_CONN_HANDLE_NONE && bleServer) {
        ...
        while (nimbleBluetoothConnHandle.load() != BLE_HS_CONN_HANDLE_NONE && Throttle::isWithinTimespanMs(started, 1000))
            delay(10);
    }
