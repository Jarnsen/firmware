...
void drawPair(OLEDDisplay *display, int left, int right, int y,
...
void finishStockPage(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x,
                     int16_t y) {
...
bool heltecV3MeshHealthPageEnabled() { return roleEnabled(); }

bool heltecV3AntennaPageEnabled() { return roleEnabled(); }
...
void heltecV3MeshHealthPageDrawFrame(OLEDDisplay *display,
  ...
  if (heltecV3MeshMonitorRecentDirect(&node, 1) == 1) {
    ...
    snprintf(r, sizeof(r), "%ddBm %+.1fdB", (int)node.rssiDbm,
             node.snrQ4 / 4.0f);
...
void heltecV3AntennaPageDrawFrame(OLEDDisplay *display,
  ...
  if (a.phase == HeltecV3AntennaPhase::A_RUNNING ||
      a.phase == HeltecV3AntennaPhase::B_RUNNING) {
