...
void heltecV3DiagNotePositionSave(bool automatic, uint32_t differenceM)
{
    ...
    saveCounters();
    heltecV3DiagLog(automatic ? "POSITION_AUTO" : "POSITION_MAN", "diff=%um auto=%u manual=%u",
                    (unsigned)differenceM, (unsigned)stats.autoPositionSaveCount,
...
void heltecV3DiagRequestUsbExport()
{
    ...
    serialConnectedSinceMs = 0;
    exportBytesSent = 0;
    heltecV3DiagLog("LOG_EXPORT", "requested serial=%u bytes=%u", (bool)Serial ? 1U : 0U,
...
{
    switch (exportState) {
    case UsbExportState::WAIT_USB: return (bool)Serial ? "USB READY" : "CONNECT PC";
    case UsbExportState::SENDING: return "LOG EXPORT";
    case UsbExportState::COMPLETE: return "LOG DONE";
    case UsbExportState::ERROR: return "LOG ERROR";
    case UsbExportState::IDLE:
    default: return "LOG READY";
    }
}
