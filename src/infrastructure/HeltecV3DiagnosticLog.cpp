...
bool heltecV3DiagStartBleExport()
{
    ...
    bleHeaderLength = (size_t)snprintf(
        ...
        "# node_id=!%08x\r\n# long_name=%s\r\n# short_name=%s\r\n# build_time=%s %s\r\n# role=%s\r\n"
        "# feature=%s\r\n# log_format=%u\r\n# export=%s\r\n# transport=BLE\r\n# bytes=%u\r\n",
        xstr(APP_VERSION), JARNSEN_V3_BUILD_SHA, (unsigned)nodeNum, longName, shortName, __DATE__, __TIME__, diagRoleText(),
        DIAG_FEATURE_VERSION, (unsigned)DIAG_LOG_FORMAT, exportTime, (unsigned)totalBytes);
