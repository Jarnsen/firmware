...
enum class UsbExportState : uint8_t {
  IDLE = 0,
  WAIT_USB,
  SENDING,
  COMPLETE,
...
bool pumpFileChunk() {
  ...
  if (available <= 0)
    return true;
  const size_t want =
      (size_t)available < sizeof(buffer) ? (size_t)available : sizeof(buffer);
...
void heltecV3DiagInit() {
  ...
  heltecV3DiagLog("BOOT",
                  ...
                  (unsigned)stats.bootCount, heltecV3DiagResetReasonText(),
                  (unsigned)stats.crashResetCount, diagRoleText(),
                  xstr(APP_VERSION), JARNSEN_V3_BUILD_SHA, __DATE__, __TIME__,
...
void heltecV3DiagNoteBleConnection() {
  ...
  if ((stats.bleConnectionCount & 7U) == 0U)
    saveCounters();
  heltecV3DiagLog("BLE_CONNECT", "count=%u",
                  (unsigned)stats.bleConnectionCount);
}
...
void heltecV3DiagNotePositionSave(bool automatic, uint32_t differenceM) {
  ...
  saveCounters();
  heltecV3DiagLog(automatic ? "POSITION_AUTO" : "POSITION_MAN",
                  "diff=%um auto=%u manual=%u", (unsigned)differenceM,
                  (unsigned)stats.autoPositionSaveCount,
                  (unsigned)stats.manualPositionSaveCount);
}

HeltecV3DiagStats heltecV3DiagStats() { return stats; }
...
void heltecV3DiagRequestUsbExport() {
  ...
  serialConnectedSinceMs = 0;
  exportBytesSent = 0;
  heltecV3DiagLog("LOG_EXPORT", "requested serial=%u bytes=%u",
                  (bool)Serial ? 1U : 0U, (unsigned)heltecV3DiagLogSize());
  exportTotalBytes = heltecV3DiagLogSize();
}

bool heltecV3DiagUsbExportPending() { return exportRequested; }
...
bool heltecV3DiagStartBleExport() {
  ...
  heltecV3DiagLog("LOG_EXPORT", "requested ble=1 bytes=%u",
                  (unsigned)heltecV3DiagLogSize());
  ...
  bleHeaderLength = (size_t)snprintf(
      ...
      "# feature=%s\r\n# log_format=%u\r\n# export=%s\r\n# transport=BLE\r\n# "
      "bytes=%u\r\n",
      xstr(APP_VERSION), JARNSEN_V3_BUILD_SHA, (unsigned)nodeNum, longName,
      shortName, __DATE__, __TIME__, diagRoleText(), DIAG_FEATURE_VERSION,
      (unsigned)DIAG_LOG_FORMAT, exportTime, (unsigned)totalBytes);
...
size_t heltecV3DiagReadBleExport(uint8_t *buffer, size_t capacity) {
  ...
  while (output < capacity && bleExportPhase != 5) {
    if (bleExportPhase == 1) {
      ...
      size_t &remaining =
          bleExportPhase == 2 ? blePreviousRemaining : bleCurrentRemaining;
      ...
      if (bleFooterLength == 0) {
        bleFooterLength = (size_t)snprintf(
            bleFooter, sizeof(bleFooter),
            "\r\n# payload_sent=%u\r\n# crc32=%08x\r\n"
      ...
      if (bleFooterOffset == bleFooterLength) {
        bleExportPhase = 5;
        closeBleExportFile();
        heltecV3DiagLog("LOG_EXPORT", "ble complete sent=%u crc=%08x",
                        (unsigned)blePayloadSent,
...
void heltecV3DiagPumpUsbExport() {
  ...
  case 1: {
    ...
    snprintf(header, sizeof(header),
             ...
             "# build_time=%s %s\r\n# role=%s\r\n# feature=%s\r\n"
             "# log_format=%u\r\n# export=%s\r\n# bytes=%u\r\n",
             xstr(APP_VERSION), JARNSEN_V3_BUILD_SHA, (unsigned)nodeNum,
             longName, shortName, __DATE__, __TIME__, diagRoleText(),
             DIAG_FEATURE_VERSION, (unsigned)DIAG_LOG_FORMAT, exportTime,
  ...
  case 4:
    ...
    exportRequested = false;
    exportPhase = 0;
    exportState = exportBytesSent >= exportTotalBytes ? UsbExportState::COMPLETE
                                                      : UsbExportState::ERROR;
    closeExportFile();
    heltecV3DiagLog("LOG_EXPORT", "%s sent=%u expected=%u",
                    exportState == UsbExportState::COMPLETE ? "complete"
                                                            : "incomplete",
...
HeltecV3DiagStats heltecV3DiagStats() { return {}; }
const char *heltecV3DiagResetReasonText() { return "N/A"; }
size_t heltecV3DiagLogSize() { return 0; }
void heltecV3DiagClear() {}
void heltecV3DiagRequestUsbExport() {}
bool heltecV3DiagUsbExportPending() { return false; }
const char *heltecV3DiagUsbExportStatusText() { return "N/A"; }
uint8_t heltecV3DiagUsbExportProgress() { return 0; }
void heltecV3DiagPumpUsbExport() {}
