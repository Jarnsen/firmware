...
static meshtastic_Position v3PendingPhonePosition =
    meshtastic_Position_init_default;
static volatile bool v3PhonePositionPending = false;
static meshtastic_Position v3LatestGoodPhonePosition =
    meshtastic_Position_init_default;
...
static meshtastic_Position v3AutoConfirmAnchor =
    meshtastic_Position_init_default;
...
static void v3UpdateUsbMaintenance() {
  ...
  LOG_INFO("Heltec V3 USB maintenance: %s",
           connected ? "active; light sleep vetoed"
                     : "closed; normal light sleep restored");
}

bool heltecV3RuntimeRoleEnabled() { return v3RepeaterRoleEnabled(); }
...
uint32_t heltecV3RuntimeServiceRemainingSecs() {
  ...
  return elapsed >= V3_SERVICE_IDLE_MS
             ? 0U
             : (V3_SERVICE_IDLE_MS - elapsed + 999U) / 1000U;
}
...
static void v3BluetoothOffNow() {
    ...
    !MESHTASTIC_EXCLUDE_BLUETOOTH
  if (nimbleBluetooth && nimbleBluetooth->isActive() &&
      !nimbleBluetooth->isAdvertisingSuppressed()) {
...
static uint32_t v3DistanceMeters(const meshtastic_Position &a,
                                 const meshtastic_Position &b) {
  ...
  const double dLon =
      (((double)b.longitude_i - (double)a.longitude_i) / 10000000.0) *
      DEG_TO_RAD_LOCAL;
...
static bool v3LoadSavedPosition(meshtastic_Position &position) {
  ...
  if (nodeDB->copyNodePosition(nodeDB->getNodeNum(), lite) &&
      (lite.latitude_i != 0 || lite.longitude_i != 0)) {
...
static bool v3PhoneFixFresh(const meshtastic_Position &position) {
  ...
  if (nowEpoch == 0)
    return true;
  const uint32_t age = nowEpoch >= position.time ? nowEpoch - position.time
                                                 : position.time - nowEpoch;
  return age <= V3_POSITION_FRESH_SECS;
}

static bool v3PhoneFixAccurate(const meshtastic_Position &position) {
  return position.gps_accuracy > 0 &&
         position.gps_accuracy <= V3_POSITION_GOOD_ACCURACY_MM;
}
...
class V3PhonePositionCaptureModule
    : public ProtobufModule<meshtastic_Position> {
public:
  V3PhonePositionCaptureModule()
      : ProtobufModule("v3-phone-position", meshtastic_PortNum_POSITION_APP,
...
protected:
  bool handleReceivedProtobuf(const meshtastic_MeshPacket &mp,
                              meshtastic_Position *position) override {
...
static V3PhonePositionCaptureModule *v3PhonePositionCaptureModule =
    &v3PhonePositionCaptureModuleInstance;
...
static void drawV3ServiceFrame(OLEDDisplay *display, OLEDDisplayUiState *state,
                               int16_t, int16_t) {
  if (!display)
    return;
  display->clear();
...
static void showV3ServicePage() {
  ...
  const uint32_t remainingMs =
      elapsed >= V3_SERVICE_IDLE_MS ? 0U : V3_SERVICE_IDLE_MS - elapsed;
  const unsigned remaining = (unsigned)(remainingMs / 1000UL);

  if (v3ServicePage == V3_PAGE_STATUS) {
    snprintf(v3ServiceBanner, sizeof(v3ServiceBanner),
             "V3 SERVICE\n%s BAT %u%%\nBT %us A%u/%u\nSHORT: NEXT", role,
             battery, remaining, (unsigned)v3BleActivityWindowCount,
    ...
    if (!v3LatestGoodPhonePositionValid) {
      ...
      snprintf(v3ServiceBanner, sizeof(v3ServiceBanner),
               "POSITION OK\nDIFF %um  ACC %um\nLONG: SAVE POS",
               (unsigned)v3LatestPhoneDifferenceM,
               (unsigned)(v3LatestPhoneAccuracyMm / 1000UL));
...
static void showV3PositionSaved(bool automatic, uint32_t differenceM,
                                bool meshSent) {
  if (!screen)
    return;
  snprintf(v3ServiceBanner, sizeof(v3ServiceBanner),
...
static bool v3SavePosition(const meshtastic_Position &phonePosition,
                           bool automatic, uint32_t differenceM) {
  if (!nodeDB || !v3PhoneFixHasCoordinates(phonePosition) ||
      !v3PhoneFixFresh(phonePosition) || !v3PhoneFixAccurate(phonePosition))
    return false;
  ...
  LOG_INFO(
      "Heltec V3 position %s: lat=%d lon=%d acc=%umm previous-diff=%um mesh=%s",
      automatic ? "auto-updated" : "manually saved", fixed.latitude_i,
      fixed.longitude_i, (unsigned)fixed.gps_accuracy, (unsigned)differenceM,
      meshSent ? "sent" : "not-sent");
...
static void v3ProcessPhonePosition(const meshtastic_Position &position) {
  ...
  const uint32_t nowEpoch = getValidTime(RTCQualityFromNet);
  const uint32_t fixAge =
      (position.time != 0 && nowEpoch != 0)
          ? (nowEpoch >= position.time ? nowEpoch - position.time
                                       : position.time - nowEpoch)
          : UINT32_MAX;
  ...
  if (!v3PhoneFixHasCoordinates(position) || !v3LatestPhoneFixFresh ||
      !v3LatestPhoneFixAccurate) {
  ...
  if (!v3AutoConfirmAnchorValid ||
      (uint32_t)(now - v3AutoConfirmStartedMs) >
          V3_POSITION_CONFIRM_WINDOW_MS ||
      v3DistanceMeters(v3AutoConfirmAnchor, position) >
          V3_POSITION_CONFIRM_CLUSTER_METERS) {
  ...
  } else if ((uint32_t)(now - v3AutoConfirmLastMs) >=
             V3_POSITION_CONFIRM_SPACING_MS) {
    v3AutoConfirmCount++;
    v3AutoConfirmLastMs = now;
  }
...
void heltecV3CapturePhonePosition(const meshtastic_Position &position) {
  ...
  LOG_INFO(
      "Heltec V3 phone GPS captured pre-router: lat=%d lon=%d acc=%umm time=%u",
      position.latitude_i, position.longitude_i,
      (unsigned)position.gps_accuracy, (unsigned)position.time);
...
bool heltecV3GetPositionUiState(HeltecV3PositionUiState &out) {
  ...
  out.havePhonePosition =
      v3LatestPhonePositionReceivedMs != 0 && v3PhoneFixHasCoordinates(phone);
  if (out.havePhonePosition) {
    ...
    if (phone.time != 0 && nowEpoch != 0)
      out.phoneAgeSecs = nowEpoch >= phone.time ? nowEpoch - phone.time
                                                : phone.time - nowEpoch;
...
bool heltecV3ManualSaveLatestPosition() {
  if (!v3RepeaterRoleEnabled() || !v3ServiceActive ||
      !v3LatestGoodPhonePositionValid)
    return false;
  meshtastic_Position saved = meshtastic_Position_init_default;
  const uint32_t differenceM =
      v3LoadSavedPosition(saved)
          ? v3DistanceMeters(saved, v3LatestGoodPhonePosition)
          : 0U;
...
static void stopV3ServiceMode() {
  ...
  heltecV3DiagLog("SERVICE_CLOSE",
                  "BLE/display parked; repeater policy restored");
...
static void v3ServiceTask(void *) {
  for (;;) {
    ...
    heltecV3PowerMonitorTick(
        !v3ServiceActive, v3ServiceActive, v3BleConnected(),
    ...
    if (!v3ServiceActive && (v3ServiceButtonEvent || pressEdge)) {
      if ((uint32_t)(now - v3LastAcceptedButtonMs) >=
          (uint32_t)V3_SERVICE_DEBOUNCE_MS) {
        v3ServiceButtonEvent = false;
        ...
        v3LongPressHandled = false;
               ...
               (uint32_t)(now - v3LastAcceptedButtonMs) >=
                   (uint32_t)V3_SERVICE_DEBOUNCE_MS) {
      v3LastAcceptedButtonMs = now ? now : 1;
      v3ButtonPressedSinceMs = now ? now : 1;
...
#ifdef BUTTON_PIN
    if (v3ButtonWasPressed && pressed && !v3OpenedServiceThisPress &&
        !v3LongPressHandled &&
    ...
    if (v3ButtonWasPressed && !pressed) {
      const uint32_t heldMs = v3ButtonPressedSinceMs != 0
                                  ? (uint32_t)(now - v3ButtonPressedSinceMs)
                                  : 0U;
      const bool validTap = heldMs >= 40UL;
      const bool actionGuardExpired =
          v3LastPageAdvanceMs == 0 ||
          (uint32_t)(now - v3LastPageAdvanceMs) >= 120UL;

      if (!v3OpenedServiceThisPress && !v3LongPressHandled && validTap &&
          actionGuardExpired) {
        if (heltecV3ServiceMenuActive()) {
          heltecV3ServiceMenuNext();
          LOG_DEBUG(
              "Heltec V3 button: one tap -> next service-menu item (held=%ums)",
              (unsigned)heldMs);
        } else if (screen) {
          screen->showNextFrame();
          screen->runNow();
          LOG_DEBUG("Heltec V3 button: one tap -> one next frame (held=%ums)",
                    (unsigned)heldMs);
        }
        ...
        v3DisplayVisible = true;
        v3ServiceLastActivityMs = now;
      } else if (!v3OpenedServiceThisPress && !v3LongPressHandled &&
                 !validTap) {
        LOG_DEBUG("Heltec V3 button: ignored bounce pulse (%ums)",
                  (unsigned)heldMs);
      } else if (!v3OpenedServiceThisPress && !v3LongPressHandled &&
                 !actionGuardExpired) {
    ...
    if (trafficNow < v3BleTrafficLast) {
      ...
      if (v3BleActivityWindowStartedMs == 0 ||
          (uint32_t)(now - v3BleActivityWindowStartedMs) >
              (uint32_t)V3_SERVICE_ACTIVITY_WINDOW_MS) {
      ...
      v3BleActivityWindowCount =
          activityCount > (uint32_t)V3_SERVICE_ACTIVITY_THRESHOLD
    ...
    const bool hardCapReached =
        (uint32_t)(now - v3ServiceStartedMs) >= (uint32_t)V3_SERVICE_MAX_MS;
    const bool idleExpired = (uint32_t)(now - v3ServiceLastActivityMs) >=
                             (uint32_t)V3_SERVICE_IDLE_MS;
    ...
    if (v3DisplayVisible && (uint32_t)(displayNow - v3DisplayStartedMs) >=
                                (uint32_t)V3_SERVICE_DISPLAY_MS) {
      v3DisplayVisible = false;
      if (screen && screen->isScreenOn())
        screen->setOn(false);
...
static void setupV3ServiceButton() {
  ...
  if (!v3ServiceTaskHandle)
    xTaskCreate(v3ServiceTask, "V3Service", 6144, nullptr, 1,
                &v3ServiceTaskHandle);
...
void lateInitVariant() {
  ...
  config.bluetooth.enabled = true;
  LOG_INFO(
      "Heltec V3 BLE: pre-initialize NimBLE once before first light sleep");
  v3BluetoothOnNow();
  v3BluetoothOffNow();
  config.bluetooth.enabled = false;
  ...
  LOG_INFO("Heltec V3 %s duty: LS + LoRa wake, BLE/WiFi/display/LED off, GPIO0 "
           ...
           "resetReason=%d",
           config.device.role == meshtastic_Config_DeviceConfig_Role_ROUTER_LATE
               ? "ROUTER_LATE repeater"
               : "legacy REPEATER",
           (unsigned)moduleConfig.telemetry.device_update_interval,
