    ...
    for (auto &entry : directNodes) {
        if (entry.nodeNum == nodeNum) {
            ...
            if (entry.avgSamples < 64U) {
                ...
                // Slowly follow changed link conditions without an unbounded accumulator.
...
{
    switch (phase) {
    case HeltecV3AntennaPhase::A_RUNNING: return "A_RUNNING";
...
bool heltecV3AntennaHandleLongPress()
{
    ...
    case HeltecV3AntennaPhase::COMPLETE: {
        ...
        heltecV3DiagLog("ANT_A_START", "ref=%s !%08x target=%u minimum=%u", name, (unsigned)ref,
    ...
    case HeltecV3AntennaPhase::A_RUNNING: {
        ...
        if (!result.valid) {
            heltecV3DiagLog("ANT_A_WAIT", "samples=%u minimum=%u", (unsigned)result.samples,
    ...
    case HeltecV3AntennaPhase::B_RUNNING: {
        const HeltecV3AntennaResult result = finalizeLive();
        if (!result.valid) {
            heltecV3DiagLog("ANT_B_WAIT", "samples=%u minimum=%u", (unsigned)result.samples,
                            (unsigned)ANT_MIN_SAMPLES);
            return true;
        }
        antennaB = result;
...
void heltecV3MeshMonitorPrintSnapshot(Print &out)
{
    ...
    out.printf("KNOWN %u\r\nACTIVE15 %u\r\nACTIVE1H %u\r\nACTIVE24H %u\r\nDIRECT15 %u\r\nRX1H %u\r\nTOTAL_RX %u\r\n",
               (unsigned)s.knownNodes, (unsigned)s.active15m, (unsigned)s.active1h, (unsigned)s.active24h,
               (unsigned)s.direct15m, (unsigned)s.rx1h, (unsigned)totalPhysicalRx);
    ...
    for (size_t i = 0; i < count; ++i) {
        out.printf("%s !%08x last=%us rssi=%ddBm snr=%+.2fdB rx=%u\r\n", nodes[i].shortName,
                   (unsigned)nodes[i].nodeNum, (unsigned)nodes[i].ageSecs, (int)nodes[i].rssiDbm,
...
size_t heltecV3MeshMonitorRecentDirect(HeltecV3DirectNodeView *, size_t) { return 0; }
