...
DirectNode *noteDirect(uint32_t nodeNum, int16_t rssi, int16_t snrQ4, uint32_t now)
{
...
uint16_t countRamActive(uint32_t windowMs, uint32_t now)
{
    ...
    for (const auto &entry : activeNodes) {
        if (entry.nodeNum && entry.lastSeenMs && (uint32_t)(now - entry.lastSeenMs) <= windowMs)
            count++;
    }
    return count;
}
...
void loadAntennaState()
{
    ...
    antennaPhase = phase <= (uint8_t)HeltecV3AntennaPhase::SWAP_LOCKED ? (HeltecV3AntennaPhase)phase : HeltecV3AntennaPhase::IDLE;
...
{
    loadAntennaState();
    if (antennaPhase != HeltecV3AntennaPhase::A_RUNNING && antennaPhase != HeltecV3AntennaPhase::B_RUNNING)
        return;
...
HeltecV3MeshSummary heltecV3MeshMonitorSummary()
{
    ...
    for (const auto &entry : directNodes) {
        if (entry.nodeNum && entry.lastSeenMs && (uint32_t)(now - entry.lastSeenMs) <= DIRECT_RECENT_MS)
            out.direct15m++;
    }
    return out;
}

size_t heltecV3MeshMonitorRecentDirect(HeltecV3DirectNodeView *out, size_t maxCount)
{
    if (!out || maxCount == 0)
        return 0;
    const uint32_t now = millis();
...
void heltecV3MeshMonitorTick()
{
    ...
    if (lastHealthLogMs == 0)
        ...
        heltecV3DiagLog("MESH_HEALTH", "known=%u active15=%u active1h=%u active24h=%u direct15=%u rx1h=%u",
                        (unsigned)s.knownNodes, (unsigned)s.active15m, (unsigned)s.active1h, (unsigned)s.active24h,
                        (unsigned)s.direct15m, (unsigned)s.rx1h);
    }

    if (!noDirectWarningLogged && lastAnyDirectMs != 0 && (uint32_t)(now - lastAnyDirectMs) >= NO_DIRECT_WARN_MS) {
...
bool heltecV3AntennaHandleLongPress()
{
    ...
    case HeltecV3AntennaPhase::COMPLETE: {
        ...
        if (!ref) {
            heltecV3DiagLog("ANT_TEST", "start rejected: no recent attributable direct node");
            return true;
        }
        ...
        heltecV3DiagLog("ANT_A_START", "ref=%s !%08x target=%u minimum=%u", name, (unsigned)ref, (unsigned)ANT_TARGET_SAMPLES,
    ...
    case HeltecV3AntennaPhase::A_RUNNING: {
        ...
        if (!result.valid) {
            heltecV3DiagLog("ANT_A_WAIT", "samples=%u minimum=%u", (unsigned)result.samples, (unsigned)ANT_MIN_SAMPLES);
            return true;
        }
        antennaA = result;
        antennaPhase = HeltecV3AntennaPhase::A_SAVED;
        saveAntennaState();
        heltecV3DiagLog("ANT_A_SAVED", "ref=!%08x n=%u rssi=%ddBm snrQ4=%d; power off before antenna swap",
                        (unsigned)antennaReferenceNode, (unsigned)antennaA.samples, (int)antennaA.medianRssiDbm,
                        (int)antennaA.medianSnrQ4);
        clearLiveSamples();
        return true;
    }
    case HeltecV3AntennaPhase::A_SAVED:
    ...
    case HeltecV3AntennaPhase::SWAP_LOCKED:
        ...
        if (!heltecV3AntennaTxSafeToSwap()) {
            heltecV3DiagLog("ANT_SWAP_WAIT", "TX lock active but an RF transmission is still in "
    ...
    case HeltecV3AntennaPhase::B_RUNNING: {
        const HeltecV3AntennaResult result = finalizeLive();
        if (!result.valid) {
            heltecV3DiagLog("ANT_B_WAIT", "samples=%u minimum=%u", (unsigned)result.samples, (unsigned)ANT_MIN_SAMPLES);
            return true;
        }
        antennaB = result;
        ...
        heltecV3DiagLog("ANT_RESULT", "ref=!%08x A=%ddBm/%d B=%ddBm/%d deltaRSSI=%+ddB deltaSNR=%+.2fdB",
                        (unsigned)antennaReferenceNode, (int)antennaA.medianRssiDbm, (int)antennaA.medianSnrQ4,
                        (int)antennaB.medianRssiDbm, (int)antennaB.medianSnrQ4, deltaRssi, deltaSnrQ4 / 4.0f);
        clearLiveSamples();
        return true;
    }
    }
...
void heltecV3MeshMonitorPrintSnapshot(Print &out)
{
    ...
    out.printf("KNOWN %u\r\nACTIVE15 %u\r\nACTIVE1H %u\r\nACTIVE24H "
               "%u\r\nDIRECT15 %u\r\nRX1H %u\r\nTOTAL_RX %u\r\n",
               (unsigned)s.knownNodes, (unsigned)s.active15m, (unsigned)s.active1h, (unsigned)s.active24h, (unsigned)s.direct15m,
    ...
    for (size_t i = 0; i < count; ++i) {
        out.printf("%s !%08x last=%us rssi=%ddBm snr=%+.2fdB rx=%u\r\n", nodes[i].shortName, (unsigned)nodes[i].nodeNum,
                   (unsigned)nodes[i].ageSecs, (int)nodes[i].rssiDbm, nodes[i].snrQ4 / 4.0f, (unsigned)nodes[i].rxCount);
    }
    ...
    out.printf("ANTENNA phase=%s ref=%s !%08x live=%u txLock=%u safeSwap=%u "
               ...
               ant.txLocked ? 1U : 0U, ant.txSafeToSwap ? 1U : 0U, (unsigned)ant.a.samples, (int)ant.a.medianRssiDbm,
               ant.a.medianSnrQ4 / 4.0f, (unsigned)ant.b.samples, (int)ant.b.medianRssiDbm, ant.b.medianSnrQ4 / 4.0f);
}
