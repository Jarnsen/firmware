...
enum class V3ServiceMenu : uint8_t {
  NONE = 0,
  ROOT,
  POWER_STATS,
  DIAG_LOG,
...
void queueMenu(V3ServiceMenu menu) { pendingMenu = menu; }

void queueAction(V3MenuAction action) { pendingAction = action; }
...
void showMenu(V3ServiceMenu menu) {
  ...
  case V3ServiceMenu::ROOT: {
    ...
    static const char *options[] = {"Back", "Power Statistics",
                                    "Diagnostic Log"};
  ...
  case V3ServiceMenu::POWER_STATS: {
    static char sourceLine[40], batteryLine[48], remainingLine[48],
        measuredLine[48];
    ...
    if (p.batteryValid)
      snprintf(batteryLine, sizeof(batteryLine), "Battery: %u%%  %.2fV",
               (unsigned)p.batteryPercent, p.voltageMv / 1000.0f);
    else
    ...
    if (p.currentValid)
      snprintf(currentLine, sizeof(currentLine), "Current: %ld mA",
               (long)p.currentMa);
    else
      snprintf(currentLine, sizeof(currentLine), "Current: --");
    if (p.currentValid && p.vbusValid)
      snprintf(powerLine, sizeof(powerLine), "Power: %u mW",
               (unsigned)p.powerMw);
    else if (p.currentValid)
...
void closeMenuInternal(bool redraw) {
  if (graphics::NotificationRenderer::current_notification_type ==
      graphics::notificationTypeEnum::selection_picker)
...
bool heltecV3ServicePageEnabled() { return roleEnabled(); }
...
bool heltecV3ServiceMenuActive() { return menuActive; }
...
void heltecV3ServiceMenuPump() {
  ...
  if (menuNeedsReopen) {
    menuNeedsReopen = false;
    showMenu(currentMenu == V3ServiceMenu::NONE ? V3ServiceMenu::ROOT
                                                : currentMenu);
  }
}

void heltecV3ServiceMenuClose() { closeMenuInternal(true); }
