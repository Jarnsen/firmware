...
static void logVehicleDiagnostics()
{
    LOG_INFO("Tracker V1.1 diag: boots=%u motionWake=%u timerWake=%u "
             ...
             (unsigned)vehicleDiag.buttonWakes, (unsigned)vehicleDiag.gpioWakes, (unsigned)vehicleDiag.confirmedMotionStarts,
...
static void registerVehicleMotionEdges(uint32_t edgeCount)
{
    ...
    if (!motionConfirmationPending || (uint32_t)(now - motionCandidateStartedMs) > (uint32_t)vehicleMotionConfirmWindowMs()) {
        motionCandidateCount = 0;
    ...
    const uint32_t needed =
        vehicleMotionConfirmCount() > motionCandidateCount ? (uint32_t)vehicleMotionConfirmCount() - motionCandidateCount : 0U;
...
static void consumeMotionEdges()
{
    ...
    if (newEdges != 0) {
        ...
        LOG_DEBUG("Tracker motion: GPIO%d +%u edge(s), candidate=%u/%u active=%u", VEHICLE_MOTION_WAKE_PIN, (unsigned)newEdges,
                  (unsigned)motionCandidateCount, (unsigned)vehicleMotionConfirmCount(),
...
static void updateMotionWakePinHealth()
{
    ...
    if (digitalRead(VEHICLE_MOTION_WAKE_PIN) == LOW) {
        ...
        } else if (!wakePinStuckLow && (uint32_t)(now - wakePinLowStartedMs) >= (uint32_t)VEHICLE_MOTION_STUCK_LOW_MS) {
...
static void requestVehicleSleep(VehicleSleepReason reason)
{
    ...
    LOG_INFO("Tracker V1.1: sleep reason=%u, interval=%us", (unsigned)vehicleDiag.lastSleepReason, (unsigned)(sleepMs / 1000U));
