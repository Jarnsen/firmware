...
void trackerDiagInit() {
  ...
  if (loggingEnabled)
    trackerDiagLog("LOGGER", "initialized enabled=1 size=%u",
                   (unsigned)trackerDiagLogSize());
}

bool trackerDiagEnabled() { return loggingEnabled; }
...
void trackerDiagLogPosition(const char *event, int32_t latitudeI,
                            ...
                            bool fresh) {
  trackerDiagLog(event, "lat=%.7f lon=%.7f age=%us sats=%u fresh=%u",
                 latitudeI * 1e-7, longitudeI * 1e-7, (unsigned)ageSecs,
...
void trackerDiagRequestUsbExport() {
  ...
  trackerDiagLog("LOG_EXPORT", "requested usb=%u bytes=%u",
                 (bool)Serial ? 1U : 0U, (unsigned)trackerDiagLogSize());
...
bool trackerDiagUsbExportPending() { return exportRequested; }
...
bool trackerDiagStartBleExport() {
  ...
  trackerDiagLog("LOG_EXPORT", "requested ble=1 bytes=%u",
                 (unsigned)trackerDiagLogSize());
  ...
  bleHeaderLength = (size_t)snprintf(
      ...
      "# feature=%s\r\n# log_format=%u\r\n# export=%s\r\n# transport=BLE\r\n# "
      "bytes=%u\r\n",
      xstr(APP_VERSION), JARNSEN_BUILD_SHA, (unsigned)nodeNum, longName,
      shortName, __DATE__, __TIME__, trackerDiagRoleText(),
      JARNSEN_DIAG_FEATURE_VERSION, (unsigned)JARNSEN_DIAG_LOG_FORMAT,
      exportTime, (unsigned)totalBytes);
  ...
  while (output < capacity && bleExportPhase != 5) {
    if (bleExportPhase == 1) {
      ...
      if (bleFooterOffset == bleFooterLength) {
        bleExportPhase = 5;
        closeBleExportFile();
        trackerDiagLog("LOG_EXPORT", "ble complete sent=%u crc=%08x",
  ...
  case 1:
    // Start on a fresh line even if normal serial logging was in progress.
    {
      ...
      snprintf(header, sizeof(header),
               ...
               "# build_time=%s %s\r\n# role=%s\r\n# feature=%s\r\n"
               "# log_format=%u\r\n# export=%s\r\n# bytes=%u\r\n",
               xstr(APP_VERSION), JARNSEN_BUILD_SHA, (unsigned)nodeNum,
               longName, shortName, __DATE__, __TIME__, trackerDiagRoleText(),
               JARNSEN_DIAG_FEATURE_VERSION, (unsigned)JARNSEN_DIAG_LOG_FORMAT,
