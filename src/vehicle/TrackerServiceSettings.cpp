...
uint8_t sanitizeIndex(uint8_t index, const T (&)[N], uint8_t fallback) {
  return index < N ? index : fallback;
}

template <size_t N>
uint8_t findValueIndex(uint16_t value, const uint16_t (&values)[N],
                       uint8_t fallback) {
...
void trackerServiceSettingsInit() {
  ...
  if (prefs.begin(PREF_NAMESPACE, true)) {
    ...
    distanceIndex =
        sanitizeIndex(prefs.getUChar("distance", 1), DISTANCE_PRESETS, 1);
    intervalIndex =
        sanitizeIndex(prefs.getUChar("interval", 0), INTERVAL_PRESETS, 0);
    movingGnssIndex =
        findValueIndex(prefs.getUShort("moveGps", 10), MOVING_GNSS_PRESETS, 1);
    parkGpsSearchIndex = findValueIndex(prefs.getUShort("gpsWait", 30),
                                        PARK_GPS_SEARCH_PRESETS, 1);
    bleIdleIndex =
        findValueIndex(prefs.getUShort("bleIdle", 120), BLE_IDLE_PRESETS, 1);
    bleHardIndex =
        findValueIndex(prefs.getUShort("bleHard", 900), BLE_HARD_PRESETS, 2);
    ...
    if (savedMinutes != 0) {
      ...
      const uint8_t oldIndex =
          sanitizeIndex(prefs.getUChar("park", 1), LEGACY_PARK_PRESETS, 1);
  ...
  LOG_INFO(
      "Tracker V1.1 settings: motion=%s (%u/%ums) distance=%um interval=%us "
      ...
      (unsigned)trackerMovingGnssSecs(), (unsigned)trackerParkGpsSearchSecs(),
...
uint16_t trackerMovingGnssSecs() {
  return MOVING_GNSS_PRESETS[movingGnssIndex];
}
uint16_t trackerParkGpsSearchSecs() {
  return PARK_GPS_SEARCH_PRESETS[parkGpsSearchIndex];
}
...
bool trackerSetMotionSensitivityIndex(uint8_t index) {
  ...
  LOG_INFO("Tracker V1.1 setting changed: motion=%s (%u/%ums)",
...
bool trackerSetSmartDistanceM(uint16_t meters) {
  ...
  trackerApplyPositionSettings();
  LOG_INFO("Tracker V1.1 setting changed: smart distance=%um",
...
bool trackerSetSmartIntervalSecs(uint16_t seconds) {
  ...
  trackerApplyPositionSettings();
  LOG_INFO("Tracker V1.1 setting changed: smart interval=%us",
...
bool trackerSetMovingGnssSecs(uint16_t seconds) {
  ...
  LOG_INFO("Tracker V1.1 setting changed: moving GNSS=%us",
           (unsigned)trackerMovingGnssSecs());
  return true;
}
...
bool trackerSetParkGpsSearchSecs(uint16_t seconds) {
  ...
  LOG_INFO("Tracker V1.1 setting changed: parked GPS search=%us",
...
bool trackerSetBleIdleTimeoutSecs(uint16_t seconds) {
  ...
  LOG_INFO("Tracker V1.1 setting changed: BLE idle timeout=%us",
           (unsigned)trackerBleIdleTimeoutSecs());
  return true;
}
...
bool trackerSetBleHardTimeoutSecs(uint16_t seconds) {
  ...
  LOG_INFO("Tracker V1.1 setting changed: BLE hard timeout=%us",
           (unsigned)trackerBleHardTimeoutSecs());
  return true;
}
...
bool trackerSetParkIntervalMinutes(uint16_t minutes) {
  ...
  trackerApplyPositionSettings();
  LOG_INFO("Tracker V1.1 setting changed: park interval=%umin effective=%us",
           (unsigned)trackerParkIntervalMinutes(),
           (unsigned)trackerEffectiveParkIntervalSecs());
  return true;
}
...
bool trackerSetIna226Enabled(bool enabled) {
  ...
  LOG_INFO("Tracker V1.1 setting changed: INA226=%s",
           ina226Enabled ? "ON" : "OFF");
  return true;
}

void trackerCycleMotionSensitivity() {
  trackerSetMotionSensitivityIndex(
      (uint8_t)((motionIndex + 1U) %
                (sizeof(MOTION_PRESETS) / sizeof(MOTION_PRESETS[0]))));
}

void trackerCycleSmartDistance() {
  distanceIndex =
      (uint8_t)((distanceIndex + 1U) %
                (sizeof(DISTANCE_PRESETS) / sizeof(DISTANCE_PRESETS[0])));
  saveByte("distance", distanceIndex);
  trackerApplyPositionSettings();
  LOG_INFO("Tracker V1.1 setting changed: smart distance=%um",
           (unsigned)trackerSmartDistanceM());
...
void trackerCycleSmartInterval() {
  intervalIndex =
      (uint8_t)((intervalIndex + 1U) %
                (sizeof(INTERVAL_PRESETS) / sizeof(INTERVAL_PRESETS[0])));
  saveByte("interval", intervalIndex);
  trackerApplyPositionSettings();
  LOG_INFO("Tracker V1.1 setting changed: smart interval=%us",
           (unsigned)trackerSmartIntervalSecs());
...
void trackerCycleParkInterval() {
  parkIndex = (uint8_t)((parkIndex + 1U) %
                        (sizeof(PARK_PRESETS) / sizeof(PARK_PRESETS[0])));
