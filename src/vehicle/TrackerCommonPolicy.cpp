...
bool usbPowered() { return powerStatus && powerStatus->getHasUSB(); }

void IRAM_ATTR motionISR() {
  const uint32_t nowUs = (uint32_t)esp_timer_get_time();
  const uint32_t lastUs = lastMotionAcceptedUs;
  if (lastUs != 0 &&
      (uint32_t)(nowUs - lastUs) < TRACKER_COMMON_MOTION_ISR_DEBOUNCE_US)
    return;
...
bool restoreRetainedPosition() {
  if (!retainedLastPositionValid || !nodeDB)
    return false;
  if (retainedLastPosition.latitude_i == 0 &&
      retainedLastPosition.longitude_i == 0)
    return false;

  meshtastic_Position restored =
      TypeConversions::ConvertToPosition(retainedLastPosition);
...
void showTrackerScreen() {
  ...
  displayWindowMs = lowBattery() ? TRACKER_COMMON_LOW_BATTERY_DISPLAY_MS
...
void startService() {
  ...
  LOG_INFO("Tracker service: GPIO0 opened native Meshtastic UI + Bluetooth; "
           ...
           (unsigned)trackerBleIdleTimeoutSecs(),
           (unsigned)TRACKER_COMMON_BLE_ACTIVITY_THRESHOLD,
...
void processBleActivity(uint32_t now) {
  ...
  if (bleBurstStartedMs == 0 || (uint32_t)(now - bleBurstStartedMs) >
                                    TRACKER_COMMON_BLE_ACTIVITY_WINDOW_MS) {
  ...
  if (bleBurstCount >= TRACKER_COMMON_BLE_ACTIVITY_THRESHOLD) {
    ...
    LOG_DEBUG(
        "Tracker service: active BLE burst detected; %us idle timer reset",
        (unsigned)trackerBleIdleTimeoutSecs());
  }
}
...
void confirmMotion(uint32_t now) {
  ...
  trackerDiagLog("MOTION", "confirmed pulses=%u window=%ums",
...
void processMotionPinHealth(uint32_t now) {
  if (digitalRead(VEHICLE_MOTION_WAKE_PIN) == LOW) {
    if (motionPinLowSinceMs == 0) {
    ...
    } else if (!motionPinStuckLow && (uint32_t)(now - motionPinLowSinceMs) >=
                                         TRACKER_COMMON_MOTION_STUCK_LOW_MS) {
      ...
      LOG_WARN("Tracker V1.1: GPIO%d LOW for %us; motion wake disabled until "
               ...
               (unsigned)(TRACKER_COMMON_MOTION_STUCK_LOW_MS / 1000UL));
    }
  } else {
    ...
    if (motionPinStuckLow) {
      ...
      LOG_INFO("Tracker V1.1: GPIO%d recovered HIGH; motion wake available",
               VEHICLE_MOTION_WAKE_PIN);
    }
  }
}
...
void processMotion(uint32_t now) {
  ...
  if (newEdges != 0) {
    LOG_DEBUG("Tracker motion: GPIO%d +%u edge(s), candidate=%u/%u active=%u",
              ...
              (unsigned)trackerMotionConfirmCount(), motionActive ? 1U : 0U);
    ...
    if (motionActive) {
      ...
      const uint32_t needed =
          trackerMotionConfirmCount() > motionCandidateCount
  ...
  if (motionCandidatePending && (uint32_t)(now - motionCandidateStartedMs) >=
                                    trackerMotionConfirmWindowMs()) {
    LOG_DEBUG("Tracker V1.1: rejected vibration candidate (%u/%u pulses)",
              (unsigned)motionCandidateCount,
              (unsigned)trackerMotionConfirmCount());
    motionCandidateCount = 0;
    motionCandidateStartedMs = 0;
...
void armDeepSleepButtonWake() {
  ...
  if (err != ESP_OK)
    LOG_ERROR(
        "Tracker V1.1: failed to enable deep-sleep GPIO0 service wake: %d",
        (int)err);
}
...
protected:
  int onNotify(void *) override {
    ...
    if (motionPinStuckLow || digitalRead(VEHICLE_MOTION_WAKE_PIN) == LOW) {
      ...
      attachInterrupt(digitalPinToInterrupt(VEHICLE_MOTION_WAKE_PIN), motionISR,
                      FALLING);
      if (!trackerUsesDeepSleep() && parked)
...
class TrackerCommonLightSleepEndObserver
...
void armDeepSleepMotionWake() {
  const gpio_num_t pin = (gpio_num_t)VEHICLE_MOTION_WAKE_PIN;
  if (!rtc_gpio_is_valid_gpio(pin) || motionPinStuckLow ||
      digitalRead(VEHICLE_MOTION_WAKE_PIN) == LOW)
    return;
  ...
  if (err != ESP_OK)
    LOG_ERROR(
        "Tracker V1.1: failed to enable deep-sleep motion wake on GPIO%d: %d",
        VEHICLE_MOTION_WAKE_PIN, err);
}

extern "C" void trackerRealDeepSleep(unsigned long, bool,
...
void enterParkedState(const char *reason) {
  ...
  if (trackerUsesDeepSleep()) {
    ...
    trackerDiagLog("PARK_SLEEP", "TAK_TRACKER reason=%s timer=%us", reason,
                   (unsigned)(sleepMs / 1000UL));
    LOG_INFO("Tracker V1.1: %s; TAK_TRACKER entering deep sleep for %us",
             reason, (unsigned)(sleepMs / 1000UL));
  ...
  if (!parked) {
    ...
    resetFinalPositionState();
    useParkedGnssPolicy();
    trackerDiagLog("PARK_ENTER", "TAK reason=%s heartbeat=%us GNSS=sleep",
...
void processStationaryFinalPosition(uint32_t now) {
  ...
  if (!finalPositionRequested) {
    ...
    if ((uint32_t)(now - finalPositionWaitStartedMs) <
        TRACKER_COMMON_FINAL_GPS_WAIT_MS)
      return;
  ...
  if ((uint32_t)(now - finalPositionRequestedAtMs) >=
      TRACKER_COMMON_SLEEP_AFTER_POSITION_MS)
...
void processDeepSleepTimerCycle(uint32_t now) {
  if (!trackerUsesDeepSleep() ||
      esp_sleep_get_wakeup_cause() != ESP_SLEEP_WAKEUP_TIMER || motionActive ||
      serviceActive)
    return;
  ...
  if (!timerPositionRequested) {
    ...
    if ((uint32_t)(now - bootActivityMs) <
  ...
  if ((uint32_t)(now - timerPositionRequestedAtMs) >=
      TRACKER_COMMON_SLEEP_AFTER_POSITION_MS)
    enterParkedState("park heartbeat complete");
}

void updateLightSleepHeartbeat() {
  if (trackerUsesDeepSleep() || !parked || motionActive || serviceActive ||
      !positionModule)
    return;
  ...
  if (!parkHeartbeatFixPending) {
    if (nowEpoch < lastPositionHeartbeatEpoch ||
        nowEpoch - lastPositionHeartbeatEpoch < heartbeatSecs)
      return;
  ...
  if (gpsFixSince(parkHeartbeatFixStartedMs) && sendFreshPosition(false)) {
    ...
    parkHeartbeatFixStartedMs = 0;
    useParkedGnssPolicy();
    trackerDiagLog("PARK_HEARTBEAT", "fresh fix TX after %us",
                   (unsigned)heartbeatSecs);
    LOG_INFO(
        "Tracker V1.1: TAK parked heartbeat sent with fresh GNSS fix after %us",
        (unsigned)heartbeatSecs);
    return;
  }

  if ((uint32_t)(millis() - parkHeartbeatFixStartedMs) >=
      ((uint32_t)trackerParkGpsSearchSecs() * 1000UL)) {
    sendBestPosition(false);
    lastPositionHeartbeatEpoch = nowEpoch;
    parkHeartbeatFixPending = false;
...
class TrackerCommonButtonWakeObserver
    : public Observer<esp_sleep_wakeup_cause_t> {
protected:
  int onNotify(esp_sleep_wakeup_cause_t cause) override {
    if (!trackerRoleEnabled())
...
protected:
  int onNotify(void *) override {
    if (!trackerRoleEnabled())
      return 0;
    return (serviceActive || motionActive || motionCandidatePending ||
            finalPositionWaitStartedMs != 0 || parkHeartbeatFixPending)
               ? 1
               : 0;
  }
};
...
protected:
  int32_t runOnce() override {
    ...
    if (serviceActive) {
      ...
      const bool hardCap = (uint32_t)(now - serviceStartedMs) >=
                           (uint32_t)trackerBleHardTimeoutSecs() * 1000UL;
      const bool idle = (uint32_t)(now - serviceLastActivityMs) >=
    ...
    const bool gnssActiveForPower = !parked || motionActive ||
                                    parkHeartbeatFixPending ||
                                    finalPositionWaitStartedMs != 0;
    trackerPowerMonitorTick(motionActive, parked, gnssActiveForPower,
                            serviceActive,
...
bool trackerCommonIsParked() { return trackerRoleEnabled() && parked; }
...
void setupTrackerCommonPolicy() {
  ...
  trackerDiagLog(
      ...
      (unsigned)trackerEffectiveParkIntervalSecs(), xstr(APP_VERSION),
      JARNSEN_BUILD_SHA, __DATE__, __TIME__, JARNSEN_DIAG_FEATURE_VERSION,
  ...
  if (esp_sleep_get_wakeup_cause() == ESP_SLEEP_WAKEUP_EXT0) {
    ...
    motionCandidatePending = true;
    parked = false;
    LOG_INFO("Tracker V1.1: GPIO%d motion wake candidate (1/%u)",
             VEHICLE_MOTION_WAKE_PIN, (unsigned)trackerMotionConfirmCount());
  }
