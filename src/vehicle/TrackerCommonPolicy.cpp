...
#endif
#define TRACKER_COMMON_LOCK_HOLD_MS 10000UL
#define TRACKER_COMMON_LOCK_COUNTDOWN_START_MS 5000UL
...
void showFullLockCountdown(uint8_t remaining)
{
...
void startService()
{
    ...
    LOG_INFO("Tracker service: GPIO0 opened native Meshtastic UI + Bluetooth; "
             ...
             (unsigned)(TRACKER_COMMON_BLE_ACTIVITY_WINDOW_MS / 1000UL), (unsigned)trackerBleHardTimeoutSecs());
}
...
void processBleActivity(uint32_t now)
{
    ...
    const uint32_t room =
        TRACKER_COMMON_BLE_ACTIVITY_THRESHOLD > bleBurstCount ? TRACKER_COMMON_BLE_ACTIVITY_THRESHOLD - bleBurstCount : 0U;
...
void processMotion(uint32_t now)
{
    ...
    if (newEdges != 0) {
        LOG_DEBUG("Tracker motion: GPIO%d +%u edge(s), candidate=%u/%u active=%u", VEHICLE_MOTION_WAKE_PIN, (unsigned)newEdges,
                  (unsigned)motionCandidateCount, (unsigned)trackerMotionConfirmCount(), motionActive ? 1U : 0U);
  ...
  protected:
    int onNotify(void *) override
    {
        ...
        if (err == ESP_OK) {
            ...
            LOG_ERROR("Tracker V1.1: failed to arm GPIO%d light-sleep motion wake: %d", VEHICLE_MOTION_WAKE_PIN, (int)err);
            attachInterrupt(digitalPinToInterrupt(VEHICLE_MOTION_WAKE_PIN), motionISR, FALLING);
        }
        return 0;
    }
        ...
        if (pressed) {
            ...
            if (serviceActive && buttonPressedSinceMs != 0) {
                ...
                if (!jarnsen::serviceSecurityLocked() && heldMs >= TRACKER_COMMON_LOCK_HOLD_MS &&
                    ...
                    lockGestureHandled = true;
                    lockCountdownLast = 0;
                } else if (!jarnsen::serviceSecurityLocked() && heldMs >= TRACKER_COMMON_LOCK_COUNTDOWN_START_MS &&
                           !lockGestureHandled) {
                    const uint8_t remaining =
                        (uint8_t)((TRACKER_COMMON_LOCK_HOLD_MS - heldMs + 999U) / 1000U);
                    if (remaining != lockCountdownLast) {
                        lockCountdownLast = remaining;
                        showFullLockCountdown(remaining);
                        trackerDiagLog("SECURITY", "LOCK_COUNTDOWN remaining=%u held_ms=%u", (unsigned)remaining,
                                       (unsigned)heldMs);
                    }
                } else if (!buttonLongHandled && heldMs >= TRACKER_COMMON_BUTTON_LONG_MS) {
                    serviceLastActivityMs = now;
                    resetDisplayWindow(now);
                    if (jarnsen::serviceSecurityLocked())
                        confirmPinDigit();
                ...
                if (serviceActive) {
                    ...
                    if (!lockGestureHandled && lockCountdownLast != 0U) {
                        trackerDiagLog("SECURITY", "LOCK_COUNTDOWN cancelled held_ms=%u",
                                       buttonPressedSinceMs != 0 ? (unsigned)(releaseNow - buttonPressedSinceMs) : 0U);
                        lockCountdownLast = 0;
                        if (screen) {
...
bool trackerCommonScreenPowerAllowed(bool)
{
    return true;
}
void trackerCommonBleActivity() {}
