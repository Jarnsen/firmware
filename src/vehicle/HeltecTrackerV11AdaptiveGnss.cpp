...
void setupVehicleAdaptiveGnss() {
  ...
  if (adaptiveLowBattery()) {
    ...
    adaptiveTimerGpsWaitMs =
        clampWait(lowBatteryTarget, VEHICLE_TIMER_GPS_LOW_BATTERY_WAIT_MS,
    ...
    adaptiveTimerGpsWaitMs = clampWait(learnedTtff + VEHICLE_TTFF_MARGIN_MS,
                                       VEHICLE_TIMER_GPS_SHORT_WAIT_MS,
                                       VEHICLE_TIMER_GPS_FULL_WAIT_MS);
  } else if (consecutiveTimerNoFixes <
             VEHICLE_TIMER_GPS_FAILURES_BEFORE_SHORT) {
  ...
  LOG_INFO("Tracker V1.1 adaptive GNSS: timerWake=%u noFixStreak=%u "
           ...
           (unsigned)parkedTimerWakeCount, (unsigned)consecutiveTimerNoFixes,
           (unsigned)learnedTtff, (unsigned)(adaptiveTimerGpsWaitMs / 1000UL),
