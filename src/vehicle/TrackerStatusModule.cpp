...
bool latLonToMgrs(int32_t latitudeI, int32_t longitudeI, char *out,
                  size_t outSize) {
  ...
  snprintf(out, outSize, "%02d%c %c%c %05d %05d", zone, band, eLetter, nLetter,
           eMeters, nMeters);
  return true;
}
...
uint32_t positionAgeSecs(const meshtastic_PositionLite &position) {
  ...
  if (gpsStatus && gpsStatus->getLastFixMillis() != 0 &&
      millis() >= gpsStatus->getLastFixMillis())
...
unsigned estimatedAccuracyMeters() {
  ...
  // fallback because GPSStatus currently exposes PDOP but not
  // HDOP/gps_accuracy.
...
public:
  ...
  void drawFrame(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x,
                 int16_t y) override {
    if (!display)
      return;
    if (state)
      trackerStatusFrameIndex = state->frameState == IN_TRANSITION &&
                                        state->transitionFrameRelationship ==
    ...
    if (!havePosition) {
      ...
      display->setFont(FONT_SMALL);
      display->drawString(center, 45 + y,
    ...
    char mgrs[32] = {};
    if (!latLonToMgrs(position.latitude_i, position.longitude_i, mgrs,
                      sizeof(mgrs))) {
      display->setFont(FONT_MEDIUM);
    ...
    const char *thirdSpace =
        secondSpace ? strchr(secondSpace + 1, ' ') : nullptr;
    if (thirdSpace) {
      const size_t prefixLen = std::min(sizeof(zoneGrid) - 1,
                                        static_cast<size_t>(thirdSpace - mgrs));
...
#if GRAPHICS_TFT_COLORING_ENABLED
    graphics::registerTFTColorRegionDirect(x, 10 + y, display->getWidth(), 52,
                                           color, bg);
#endif
  }
};
...
public:
  ...
  void drawFrame(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x,
    ...
    if (power.batteryValid)
      snprintf(line, sizeof(line), "Bat:%u%% %u.%02uV",
               (unsigned)power.batteryPercent,
               (unsigned)(power.voltageMv / 1000U),
    ...
    if (power.usbPowered || power.charging)
      ...
      trackerPowerFormatDuration(power.remainingSecs, duration,
                                 sizeof(duration));
    ...
    if (nextTx == UINT32_MAX) {
      ...
      snprintf(next, sizeof(next), "Next:%um ",
               (unsigned)((nextTx + 59U) / 60U));
    } else {
      const uint32_t hours = nextTx / 3600U;
      const uint32_t mins = (nextTx % 3600U) / 60U;
      snprintf(next, sizeof(next), "Next:%uh%02um ", (unsigned)hours,
               (unsigned)mins);
    }
...
public:
  ...
  void drawFrame(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x,
    ...
    display->drawString(left, textPos[3], line);
    display->setTextAlignment(TEXT_ALIGN_RIGHT);
    snprintf(line, sizeof(line), "Search:%us",
             (unsigned)trackerParkGpsSearchSecs());
    display->drawString(right, textPos[3], line);
...
void showTrackerOptions(const char *title, const char **options, uint8_t count,
                        int selected, std::function<void(int)> callback) {
  if (!screen)
    return;
...
void refreshTrackerLogExportText() {
  snprintf(trackerLogExportStatus, sizeof(trackerLogExportStatus), "Status: %s",
           trackerDiagUsbExportStatusText());
  const uint8_t progress = trackerDiagUsbExportProgress();
  if (trackerDiagUsbExportPending() || progress > 0)
    snprintf(trackerLogExportProgress, sizeof(trackerLogExportProgress),
             "Fortschritt: %u%%", (unsigned)progress);
  else
    snprintf(trackerLogExportProgress, sizeof(trackerLogExportProgress),
             "Log: %u KB", (unsigned)((trackerDiagLogSize() + 1023U) / 1024U));
}
...
void showTrackerMenu(TrackerMenu menu, int initialSelection) {
  ...
  case TrackerMenu::ROOT: {
    static const char *opts[] = {"Back",    "Position",  "Motion",
                                 "Parking", "Bluetooth", "Diagnostic Log",
                                 "System"};
    showTrackerOptions("Service Settings", opts, 7, initialSelection,
                       [](int selected) {
  ...
  case TrackerMenu::POSITION: {
    static const char *opts[] = {"Back", "Smart Distance", "Min TX Interval",
                                 "Moving GNSS"};
  ...
  case TrackerMenu::DISTANCE: {
    ...
    static const char *opts[5] = {labels[0], labels[1], labels[2], labels[3],
                                  labels[4]};
    snprintf(labels[0], sizeof(labels[0]), "Back");
    const uint16_t vals[] = {50, 75, 100, 150};
    for (int i = 0; i < 4; ++i) {
      char raw[12];
      snprintf(raw, sizeof(raw), "%u m", (unsigned)vals[i]);
      markOption(labels[i + 1], sizeof(labels[i + 1]),
                 trackerSmartDistanceM() == vals[i], raw);
    }
    showTrackerOptions(
        "Smart Distance", opts, 5, initialSelection, [](int selected) {
          if (selected == 0)
            queueTrackerMenu(TrackerMenu::POSITION, trackerPositionSelection);
          else {
            const uint16_t vals[] = {50, 75, 100, 150};
            trackerSetSmartDistanceM(vals[selected - 1]);
            queueTrackerMenu(TrackerMenu::DISTANCE, 0);
          }
        });
    break;
  ...
  case TrackerMenu::INTERVAL: {
    ...
    for (int i = 0; i < 4; ++i) {
      char raw[12];
      snprintf(raw, sizeof(raw), "%u s", (unsigned)vals[i]);
      markOption(labels[i + 1], sizeof(labels[i + 1]),
                 trackerSmartIntervalSecs() == vals[i], raw);
    }
    showTrackerOptions(
        "Min TX Interval", opts, 5, initialSelection, [](int selected) {
          if (selected == 0)
            queueTrackerMenu(TrackerMenu::POSITION, trackerPositionSelection);
          else {
            const uint16_t vals[] = {30, 45, 60, 90};
            trackerSetSmartIntervalSecs(vals[selected - 1]);
            queueTrackerMenu(TrackerMenu::INTERVAL, 0);
          }
        });
    break;
  ...
  case TrackerMenu::MOVING_GNSS: {
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      snprintf(raw, sizeof(raw), "%u s", (unsigned)vals[i]);
      markOption(labels[i + 1], sizeof(labels[i + 1]),
                 trackerMovingGnssSecs() == vals[i], raw);
    }
    showTrackerOptions(
        "Moving GNSS", opts, 5, initialSelection, [](int selected) {
          if (selected == 0)
            queueTrackerMenu(TrackerMenu::POSITION, trackerPositionSelection);
          else {
            const uint16_t vals[] = {5, 10, 15, 30};
            trackerSetMovingGnssSecs(vals[selected - 1]);
            queueTrackerMenu(TrackerMenu::MOVING_GNSS, 0);
          }
        });
    break;
  ...
  case TrackerMenu::MOTION_SENS: {
    ...
    for (int i = 0; i < 4; ++i)
      markOption(labels[i + 1], sizeof(labels[i + 1]),
                 trackerMotionSensitivityIndex() == i, names[i]);
    showTrackerOptions(
        "Sensitivity", opts, 5, initialSelection, [](int selected) {
          if (selected == 0)
            queueTrackerMenu(TrackerMenu::MOTION, trackerMotionSelection);
          else {
            trackerSetMotionSensitivityIndex((uint8_t)(selected - 1));
            queueTrackerMenu(TrackerMenu::MOTION_SENS, 0);
          }
        });
    break;
  ...
  case TrackerMenu::PARK_POWER: {
    ...
    showTrackerOptions("Parking", opts, 3, initialSelection, [](int selected) {
      ...
      if (selected == 0)
        ...
        queueTrackerMenu(TrackerMenu::PARK_GPS_SEARCH, 0);
    });
    break;
  }
  ...
  case TrackerMenu::PARK_INTERVAL: {
    ...
    static const char *opts[9] = {labels[0], labels[1], labels[2],
                                  labels[3], labels[4], labels[5],
                                  labels[6], labels[7], labels[8]};
    const uint16_t vals[] = {20, 30, 60, 120, 240, 360, 540, 720};
    const char *names[] = {"20 min", "30 min", "60 min", "2 h",
                           "4 h",    "6 h",    "9 h",    "12 h"};
    snprintf(labels[0], sizeof(labels[0]), "Back");
    for (int i = 0; i < 8; ++i)
      markOption(labels[i + 1], sizeof(labels[i + 1]),
                 trackerParkIntervalMinutes() == vals[i], names[i]);
    showTrackerOptions(
        "Park Interval", opts, 9, initialSelection, [](int selected) {
          if (selected == 0)
            queueTrackerMenu(TrackerMenu::PARK_POWER, trackerParkSelection);
          else {
            const uint16_t vals[] = {20, 30, 60, 120, 240, 360, 540, 720};
            trackerSetParkIntervalMinutes(vals[selected - 1]);
            queueTrackerMenu(TrackerMenu::PARK_INTERVAL, 0);
          }
        });
    break;
  ...
  case TrackerMenu::PARK_GPS_SEARCH: {
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      snprintf(raw, sizeof(raw), "%u s", (unsigned)vals[i]);
      markOption(labels[i + 1], sizeof(labels[i + 1]),
                 trackerParkGpsSearchSecs() == vals[i], raw);
    }
    showTrackerOptions(
        "GPS Search Time", opts, 5, initialSelection, [](int selected) {
          if (selected == 0)
            queueTrackerMenu(TrackerMenu::PARK_POWER, trackerParkSelection);
          else {
            const uint16_t vals[] = {15, 30, 45, 60};
            trackerSetParkGpsSearchSecs(vals[selected - 1]);
            queueTrackerMenu(TrackerMenu::PARK_GPS_SEARCH, 0);
          }
        });
    break;
  ...
  case TrackerMenu::BLUETOOTH: {
    ...
    showTrackerOptions(
        "Bluetooth", opts, 3, initialSelection, [](int selected) {
          trackerBluetoothSelection = selected;
          if (selected == 0)
            queueTrackerMenu(TrackerMenu::ROOT, trackerRootSelection);
          else if (selected == 1)
            queueTrackerMenu(TrackerMenu::BLE_IDLE, 0);
          else if (selected == 2)
            queueTrackerMenu(TrackerMenu::BLE_HARD, 0);
        });
    break;
  }
  ...
  case TrackerMenu::BLE_IDLE: {
    ...
    for (int i = 0; i < 4; ++i)
      markOption(labels[i + 1], sizeof(labels[i + 1]),
                 trackerBleIdleTimeoutSecs() == vals[i], names[i]);
    showTrackerOptions(
        "Idle Timeout", opts, 5, initialSelection, [](int selected) {
          if (selected == 0)
            queueTrackerMenu(TrackerMenu::BLUETOOTH, trackerBluetoothSelection);
          else {
            const uint16_t vals[] = {60, 120, 180, 300};
            trackerSetBleIdleTimeoutSecs(vals[selected - 1]);
            queueTrackerMenu(TrackerMenu::BLE_IDLE, 0);
          }
        });
    break;
  ...
  case TrackerMenu::BLE_HARD: {
    ...
    for (int i = 0; i < 4; ++i)
      markOption(labels[i + 1], sizeof(labels[i + 1]),
                 trackerBleHardTimeoutSecs() == vals[i], names[i]);
    showTrackerOptions(
        "Hard Timeout", opts, 5, initialSelection, [](int selected) {
          if (selected == 0)
            queueTrackerMenu(TrackerMenu::BLUETOOTH, trackerBluetoothSelection);
          else {
            const uint16_t vals[] = {300, 600, 900, 1800};
            trackerSetBleHardTimeoutSecs(vals[selected - 1]);
            queueTrackerMenu(TrackerMenu::BLE_HARD, 0);
          }
        });
    break;
  ...
  case TrackerMenu::DIAG_LOG: {
    static const char *opts[] = {"Back", "Logging", "Log Status",
                                 "Export via USB", "Clear Log"};
    showTrackerOptions(
        "Diagnostic Log", opts, 5, initialSelection, [](int selected) {
          ...
          if (selected == 0)
            ...
            queueTrackerMenu(TrackerMenu::LOG_STATUS, 0);
          else if (selected == 3) {
            queueTrackerMenu(TrackerMenu::LOG_EXPORT_CONFIRM, 0);
          } else if (selected == 4)
            queueTrackerMenu(TrackerMenu::LOG_CLEAR, 0);
        });
    break;
  }
  ...
  case TrackerMenu::LOGGING: {
    ...
    showTrackerOptions(
        "Diagnostic Logging", opts, 3, initialSelection, [](int selected) {
          if (selected == 0)
            queueTrackerMenu(TrackerMenu::DIAG_LOG, trackerDiagSelection);
          else {
            trackerDiagSetEnabled(selected == 2);
            queueTrackerMenu(TrackerMenu::LOGGING, 0);
          }
        });
    break;
  ...
  case TrackerMenu::LOG_STATUS: {
    ...
    snprintf(exportLine, sizeof(exportLine), "USB export: %s",
             trackerDiagUsbExportPending() ? "WAIT/RUN" : "READY");
    showTrackerOptions(
        "Log Status", opts, 3, initialSelection, [](int selected) {
          if (selected == 0)
            queueTrackerMenu(TrackerMenu::DIAG_LOG, trackerDiagSelection);
          else
            queueTrackerMenu(TrackerMenu::LOG_STATUS, 0);
  ...
  case TrackerMenu::LOG_EXPORT: {
    static const char *opts[] = {"Back", trackerLogExportStatus,
                                 trackerLogExportProgress};
  ...
  case TrackerMenu::LOG_CLEAR: {
    ...
    showTrackerOptions(
        "Clear Diagnostic Log?", opts, 2, initialSelection, [](int selected) {
          if (selected == 0) {
            queueTrackerMenu(TrackerMenu::DIAG_LOG, trackerDiagSelection);
          } else {
            trackerDiagClear();
  ...
  case TrackerMenu::SYSTEM: {
    ...
    showTrackerOptions("System", opts, 6, initialSelection, [](int selected) {
      if (selected == 0)
        ...
        queueTrackerMenu(TrackerMenu::SYSTEM_INFO, 0);
      else if (selected == 2)
        queueTrackerMenu(TrackerMenu::DIAGNOSTICS, 0);
      else if (selected == 3)
        queueTrackerMenu(TrackerMenu::POWER_STATS, 0);
      else if (selected == 4)
  ...
  case TrackerMenu::SYSTEM_INFO: {
    ...
    snprintf(role, sizeof(role), "Role: %s",
             config.device.role ==
                     meshtastic_Config_DeviceConfig_Role_TAK_TRACKER
                 ? "TAK_TRACKER"
                 : "TAK");
    showTrackerOptions("System Info", opts, 4, initialSelection,
                       [](int selected) {
                         if (selected == 0)
                           queueTrackerMenu(TrackerMenu::SYSTEM, 1);
                         else
                           queueTrackerMenu(TrackerMenu::SYSTEM_INFO, 0);
                       });
    break;
  ...
  case TrackerMenu::DIAGNOSTICS: {
    ...
    if (age == UINT32_MAX)
      snprintf(gpsAge, sizeof(gpsAge), "GPS age: ?");
    else
      snprintf(gpsAge, sizeof(gpsAge), "GPS age: %us", (unsigned)age);
    snprintf(sensor, sizeof(sensor), "Sensor: %s", trackerMotionSensorStatus());
    snprintf(wake, sizeof(wake), "Wake: %s", trackerBootWakeReason());
    snprintf(mode, sizeof(mode), "Sleep: %s",
             config.device.role ==
                     meshtastic_Config_DeviceConfig_Role_TAK_TRACKER
                 ? "Deep"
                 : "Light");
    showTrackerOptions("Diagnostics", opts, 6, initialSelection,
                       [](int selected) {
                         if (selected == 0)
                           queueTrackerMenu(TrackerMenu::SYSTEM, 2);
                         else
                           queueTrackerMenu(TrackerMenu::DIAGNOSTICS, 0);
                       });
    break;
  ...
  case TrackerMenu::INA226_HW: {
    ...
    showTrackerOptions("INA226 Hardware", opts, 3, initialSelection,
                       [](int selected) {
                         if (selected == 0)
  ...
  case TrackerMenu::POWER_STATS: {
    ...
    if (p.batteryValid)
      snprintf(batteryLine, sizeof(batteryLine), "Battery: %u%%  %u.%03uV",
               (unsigned)p.batteryPercent, (unsigned)(p.voltageMv / 1000U),
    ...
    if (!p.inaConfigured)
      snprintf(inaLine, sizeof(inaLine), "INA226: OFF");
    else if (!p.inaPresent)
      snprintf(inaLine, sizeof(inaLine), "INA226: MISSING");
    else if (!p.inaValid)
      snprintf(inaLine, sizeof(inaLine), "INA226: WAIT");
    else if (!p.vbusValid)
      snprintf(inaLine, sizeof(inaLine), "INA226: VBUS MISSING");
    else
      snprintf(inaLine, sizeof(inaLine), "INA226: ACTIVE  %umV",
               (unsigned)p.inaBusVoltageMv);

    if (p.inaValid) {
      const int32_t c = p.currentMilliAmpsX10;
      const int32_t ac = c < 0 ? -c : c;
      snprintf(currentLine, sizeof(currentLine), "Current: %s%ld.%ld mA",
               c < 0 ? "-" : "", (long)(ac / 10), (long)(ac % 10));
    } else {
    ...
    if (p.inaValid && p.vbusValid) {
      ...
      snprintf(powerLine, sizeof(powerLine), "Power: %s%ld.%ld mW",
               w < 0 ? "-" : "", (long)(aw / 10), (long)(aw % 10));
    } else if (p.inaValid) {
    ...
    if (p.vbusValid)
      snprintf(usedLine, sizeof(usedLine), "Used: %u.%u mAh / %u.%u mWh",
               ...
               (unsigned)(p.dischargedMahX10 % 10U),
               (unsigned)(p.dischargedMwhX10 / 10U),
               (unsigned)(p.dischargedMwhX10 % 10U));
    else
    ...
    snprintf(txLine, sizeof(txLine), "Position TX: %u",
  ...
  case TrackerMenu::ANTENNA_TEST: {
    static char stateLine[40], refLine[40], sampleLine[40], aLine[48],
        bLine[48], safetyLine[48], resultLine[48], actionLine[48];
    static const char *opts[] = {"Back",     stateLine,  refLine,
                                 sampleLine, aLine,      bLine,
                                 safetyLine, resultLine, actionLine};
    const TrackerAntennaState a = trackerAntennaState();

    snprintf(stateLine, sizeof(stateLine), "State: %s",
             trackerAntennaPhaseText(a.phase));
    if (a.referenceNode)
      snprintf(refLine, sizeof(refLine), "Ref: %s !%04x", a.referenceName,
               (unsigned)(a.referenceNode & 0xffffU));
    else
      snprintf(refLine, sizeof(refLine), "Ref: last direct on start");

    if (a.phase == TrackerAntennaPhase::A_RUNNING ||
        a.phase == TrackerAntennaPhase::B_RUNNING)
      snprintf(sampleLine, sizeof(sampleLine), "Samples: %u/60  min 40",
               (unsigned)a.liveSamples);
    else
      snprintf(sampleLine, sizeof(sampleLine), "Samples: min 40 / target 60");

    if (a.a.valid)
      snprintf(aLine, sizeof(aLine), "A: %ddBm  SNR %+.1fdB",
               (int)a.a.medianRssiDbm, a.a.medianSnrQ4 / 4.0f);
    else
      snprintf(aLine, sizeof(aLine), "A: --");
    if (a.b.valid)
      snprintf(bLine, sizeof(bLine), "B: %ddBm  SNR %+.1fdB",
               (int)a.b.medianRssiDbm, a.b.medianSnrQ4 / 4.0f);
    else
      snprintf(bLine, sizeof(bLine), "B: --");

    if (a.txLocked)
      snprintf(safetyLine, sizeof(safetyLine), "TX: LOCKED %s",
               a.txSafeToSwap ? "SAFE" : "WAIT");
    else
    ...
    if (a.a.valid && a.b.valid) {
      const char *winner = a.deltaRssiDb >= 3    ? "B MUCH BETTER"
                           ...
                           : a.deltaRssiDb <= -1 ? "A BETTER"
                                                 : "ABOUT EQUAL";
      snprintf(resultLine, sizeof(resultLine), "Delta: %+ddB  %s",
               (int)a.deltaRssiDb, winner);
    } else {
    ...
    case TrackerAntennaPhase::A_RUNNING:
      snprintf(actionLine, sizeof(actionLine), "ACTION: %s",
               a.liveSamples >= 40 ? "SAVE A" : "CHECK A");
      break;
    ...
    case TrackerAntennaPhase::SWAP_LOCKED:
      snprintf(actionLine, sizeof(actionLine), "ACTION: %s",
               a.txSafeToSwap ? "B CONNECTED / START B" : "WAIT TX FINISH");
      break;
    case TrackerAntennaPhase::B_RUNNING:
      snprintf(actionLine, sizeof(actionLine), "ACTION: %s",
               a.liveSamples >= 40 ? "SAVE B" : "CHECK B");
      break;
    ...
    showTrackerOptions(
        "Antenna Test", opts, 9, initialSelection, [](int selected) {
          if (selected == 0) {
...
bool trackerServiceMenuActive() { return trackerServiceMenuMode; }
...
void trackerServiceMenuPump() {
  ...
  if (trackerMenuCurrent == TrackerMenu::LOG_EXPORT) {
    ...
    if (trackerLogExportLastRefreshMs == 0 ||
        (uint32_t)(now - trackerLogExportLastRefreshMs) >= 250U) {
...
const char *trackerStatusCurrentPageText() { return "off"; }
void trackerServiceMenuOpen() {}
void trackerServiceMenuShortPress() {}
void trackerServiceMenuSelect() {}
void trackerServiceMenuPump() {}
