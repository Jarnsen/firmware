    ...
    void drawFrame(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y) override
    {
        ...
        const auto drawCapacity = [&]() {
            if (!power.batteryValid || !power.capacityReady)
                return;
            char capacity[24] = {};
            snprintf(capacity, sizeof(capacity), "C:%u.%uAh", (unsigned)(power.learnedCapacityMah / 1000U),
        ...
        char status[48] = {};
        if (power.batteryValid && power.capacityReady)
            snprintf(status, sizeof(status), "%s | %s | C:%u.%uAh", gpsLock ? "FIX" : "LAST",
                     trackerMotionActive ? "MOVE" : "PARK", (unsigned)(power.learnedCapacityMah / 1000U),
                     (unsigned)((power.learnedCapacityMah % 1000U) / 100U));
        else
  ...
  public:
    ...
    void drawFrame(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y) override
    {
        ...
        if (power.batteryValid)
            snprintf(line, sizeof(line), "Bat:%u%% %u.%02uV", (unsigned)power.batteryPercent, (unsigned)(power.voltageMv / 1000U),
...
void showTrackerOptions(const char *title, const char **options, uint8_t count, int selected, std::function<void(int)> callback)
{
    if (!screen)
        return;
...
void showTrackerMenu(TrackerMenu menu, int initialSelection)
{
    ...
    case TrackerMenu::PARK_INTERVAL: {
        ...
        static const char *opts[9] = {labels[0], labels[1], labels[2], labels[3], labels[4],
    ...
    case TrackerMenu::BLE_IDLE: {
        ...
        for (int i = 0; i < 4; ++i)
            markOption(labels[i + 1], sizeof(labels[i + 1]), trackerBleIdleTimeoutSecs() == vals[i], names[i]);
    ...
    case TrackerMenu::BLE_HARD: {
        ...
        for (int i = 0; i < 4; ++i)
            markOption(labels[i + 1], sizeof(labels[i + 1]), trackerBleHardTimeoutSecs() == vals[i], names[i]);
    ...
    case TrackerMenu::LOG_CLEARED: {
        ...
        showTrackerOptions("LOG CLEARED", opts, 1, 0, [](int) { queueTrackerMenu(TrackerMenu::DIAG_LOG, trackerDiagSelection); });
        break;
    }
    ...
    case TrackerMenu::DIAGNOSTICS: {
        ...
        snprintf(mode, sizeof(mode), "Sleep: %s",
                 config.device.role == meshtastic_Config_DeviceConfig_Role_TAK_TRACKER ? "Deep" : "Light");
    ...
    case TrackerMenu::POWER_STATS: {
        ...
        static char usedLine[48], capacityLine[48], remainingMahLine[48], confidenceLine[48], measuredLine[48], movingLine[48];
        static char parkedLine[48], gnssLine[48], bleLine[48], displayLine[48], txLine[48], trendLine[48];
        static const char *opts[] = {"Back",     batteryLine,  remainingLine,    inaLine,        currentLine,  powerLine,
                                     usedLine,   capacityLine, remainingMahLine, confidenceLine, measuredLine, movingLine,
                                     parkedLine, gnssLine,     bleLine,          displayLine,    txLine,       trendLine};
        ...
        if (!p.inaConfigured)
            ...
            snprintf(inaLine, sizeof(inaLine), "INA226: ACTIVE  %umV", (unsigned)p.inaBusVoltageMv);

        if (p.inaValid) {
            const int32_t c = p.currentMilliAmpsX10;
            const int32_t ac = c < 0 ? -c : c;
            snprintf(currentLine, sizeof(currentLine), "Current: %s%ld.%ld mA", c < 0 ? "-" : "", (long)(ac / 10),
                     (long)(ac % 10));
        } else {
        ...
        if (p.inaValid && p.vbusValid) {
            ...
            snprintf(powerLine, sizeof(powerLine), "Power: %s%ld.%ld mW", w < 0 ? "-" : "", (long)(aw / 10), (long)(aw % 10));
        } else if (p.inaValid) {
        ...
        if (p.vbusValid)
            snprintf(usedLine, sizeof(usedLine), "Used: %u.%u mAh / %u.%u mWh", (unsigned)(p.dischargedMahX10 / 10U),
                     (unsigned)(p.dischargedMahX10 % 10U), (unsigned)(p.dischargedMwhX10 / 10U),
                     (unsigned)(p.dischargedMwhX10 % 10U));
        else
            snprintf(usedLine, sizeof(usedLine), "Used: %u.%u mAh / -- mWh", (unsigned)(p.dischargedMahX10 / 10U),
        ...
        showTrackerOptions("Power Statistics", opts, 18, initialSelection, [](int selected) {
            if (selected == 0)
    ...
    case TrackerMenu::ANTENNA_TEST: {
        static char stateLine[40], refLine[40], sampleLine[40], aLine[48], bLine[48], safetyLine[48], resultLine[48],
            actionLine[48];
