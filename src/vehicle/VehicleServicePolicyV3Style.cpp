...
static bool v3TrackerDisplayWindowActive() {
  if (!v3TrackerServiceActive || !v3TrackerDisplayVisible ||
      v3TrackerDisplayStartedMs == 0)
    return false;
  return (uint32_t)(millis() - v3TrackerDisplayStartedMs) <
         v3TrackerDisplayWindowMs;
}
...
static void drawV3TrackerServiceFrame(OLEDDisplay *display,
...
static void v3TrackerBuildPage() {
  ...
  case VEHICLE_V3_PAGE_STATUS:
    snprintf(v3TrackerBanner, sizeof(v3TrackerBanner),
             "TRACKER SERVICE\nBAT %u%%  GPS %s\nSHORT: NEXT\nBT SERVICE",
             battery, v3TrackerPositionKnown() ? "FIX" : "WAIT");
    break;
  case VEHICLE_V3_PAGE_DIAG:
    snprintf(v3TrackerBanner, sizeof(v3TrackerBanner),
             "DIAG GPS %s\nFIX %us TTFF %us\nSENSOR %s M%u",
             v3TrackerPositionKnown() ? "FIX" : "WAIT",
             v3TrackerDisplayAge(trackerLastFixAgeSecs()),
...
static void v3TrackerShowPage() {
  ...
  v3TrackerDisplayWindowMs = v3TrackerLowBattery()
                                 ? VEHICLE_V3_LOW_BATTERY_DISPLAY_MS
                                 : VEHICLE_V3_SERVICE_DISPLAY_MS;
...
static void v3TrackerStartService() {
  ...
  LOG_INFO("Tracker service: GPIO0 opened display/Bluetooth; idle=%us "
           "activity=%u/%us hard-cap=%us power-save=%s",
...
static void v3TrackerStopService() {
  ...
  if (screen && screen->isScreenOn())
    screen->setOn(false);
  LOG_INFO("Tracker service: window complete; Bluetooth/display off, "
...
protected:
  int32_t runOnce() override {
    ...
    if (!v3TrackerBootHandoffComplete && graphics::isBootScreenComplete()) {
      ...
      LOG_INFO("TAK_TRACKER: Meshtastic boot screen complete; custom display "
               "ownership active");
      if (v3TrackerServiceActive)
        v3TrackerShowPage();
    ...
    if (pressed) {
      ...
      if (!v3TrackerButtonWasPressed) {
        ...
        if (!v3TrackerServiceActive) {
          ...
          v3TrackerServiceLastActivityMs = now;
          if (!v3TrackerDisplayWindowActive() ||
      ...
      if (v3TrackerServiceActive && !v3TrackerOpenedServiceThisPress &&
          !v3TrackerLongPressHandled &&
          (uint32_t)(now - v3TrackerButtonPressedSinceMs) >=
              (uint32_t)VEHICLE_V3_LONG_PRESS_MS) {
      ...
      if ((uint32_t)(now - v3TrackerButtonHighSinceMs) >= 25U) {
        if (v3TrackerServiceActive && !v3TrackerOpenedServiceThisPress &&
            !v3TrackerLongPressHandled) {
          v3TrackerServicePage =
              (uint8_t)((v3TrackerServicePage + 1U) % VEHICLE_V3_PAGE_COUNT);
          v3TrackerShowPage();
          v3TrackerServiceLastActivityMs = now;
          LOG_DEBUG("TAK_TRACKER: GPIO0 short press -> page %u/%u",
                    (unsigned)(v3TrackerServicePage + 1U),
                    (unsigned)VEHICLE_V3_PAGE_COUNT);
        }
    ...
    if (trafficNow < v3TrackerBleTrafficLast) {
      ...
      const uint32_t activityCount =
          (uint32_t)v3TrackerBleActivityWindowCount + delta;
      ...
      if (v3TrackerServiceActive &&
        ...
        LOG_DEBUG("Tracker service: active BLE burst detected; 120s idle timer "
                  "reset");
        v3TrackerBleActivityWindowStartedMs = now ? now : 1;
        v3TrackerBleActivityWindowCount = 0;
      }
    ...
    const bool hardCapReached = (uint32_t)(now - v3TrackerServiceStartedMs) >=
                                (uint32_t)VEHICLE_V3_SERVICE_MAX_MS;
    const bool idleExpired = (uint32_t)(now - v3TrackerServiceLastActivityMs) >=
                             (uint32_t)VEHICLE_V3_SERVICE_IDLE_MS;
    ...
    if (v3TrackerDisplayVisible && v3TrackerBootHandoffComplete &&
        (v3TrackerLastFrameAssertMs == 0 ||
         (uint32_t)(frameNow - v3TrackerLastFrameAssertMs) >=
             (uint32_t)VEHICLE_V3_FRAME_REASSERT_MS)) {
    ...
    if (v3TrackerDisplayVisible &&
        (uint32_t)(displayNow - v3TrackerDisplayStartedMs) >=
            v3TrackerDisplayWindowMs) {
      v3TrackerDisplayVisible = false;
      if (screen && screen->isScreenOn())
        screen->setOn(false);
...
void setupVehicleServicePolicyV3Style() {
  ...
  if (v3TrackerBootWasUserWake()) {
    v3TrackerStartService();
    v3TrackerOpenedServiceThisPress = true;
    v3TrackerButtonWasPressed =
        button != GPIO_NUM_NC && digitalRead(button) == LOW;
    v3TrackerButtonPressedSinceMs = millis();
  }

  LOG_INFO("Tracker V1.1 TAK_TRACKER V3-style service enabled: power-save on, "
           "exclusive pages, BLE only via GPIO0");
}
