...
static void v3TrackerStartService()
{
    ...
    LOG_INFO("Tracker service: GPIO0 opened display/Bluetooth; idle=%us "
             ...
             (unsigned)(VEHICLE_V3_SERVICE_IDLE_MS / 1000UL), (unsigned)VEHICLE_V3_SERVICE_ACTIVITY_THRESHOLD,
             (unsigned)(VEHICLE_V3_SERVICE_ACTIVITY_WINDOW_MS / 1000UL), (unsigned)(VEHICLE_V3_SERVICE_MAX_MS / 1000UL),
    ...
    int32_t runOnce() override
    {
        ...
        if (pressed) {
            ...
            if ((uint32_t)(now - v3TrackerButtonHighSinceMs) >= 25U) {
                if (v3TrackerServiceActive && !v3TrackerOpenedServiceThisPress && !v3TrackerLongPressHandled) {
                    ...
                    v3TrackerShowPage();
                    v3TrackerServiceLastActivityMs = now;
                    LOG_DEBUG("TAK_TRACKER: GPIO0 short press -> page %u/%u", (unsigned)(v3TrackerServicePage + 1U),
        ...
        if (trafficNow < v3TrackerBleTrafficLast) {
            ...
            if (v3TrackerBleActivityWindowStartedMs == 0 ||
                (uint32_t)(now - v3TrackerBleActivityWindowStartedMs) > (uint32_t)VEHICLE_V3_SERVICE_ACTIVITY_WINDOW_MS) {
            ...
            v3TrackerBleActivityWindowCount = activityCount > (uint32_t)VEHICLE_V3_SERVICE_ACTIVITY_THRESHOLD
                                                  ? (uint8_t)VEHICLE_V3_SERVICE_ACTIVITY_THRESHOLD
                                                  : (uint8_t)activityCount;

            if (v3TrackerServiceActive && v3TrackerBleActivityWindowCount >= (uint8_t)VEHICLE_V3_SERVICE_ACTIVITY_THRESHOLD) {
                v3TrackerServiceLastActivityMs = now;
        ...
        if (v3TrackerDisplayVisible && (uint32_t)(displayNow - v3TrackerDisplayStartedMs) >= v3TrackerDisplayWindowMs) {
            v3TrackerDisplayVisible = false;
            if (screen && screen->isScreenOn())
                screen->setOn(false);
