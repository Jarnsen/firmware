...
void trackerAntennaTestInit() {
  ...
  if (storedLock) {
    ...
    else if (phase == TrackerAntennaPhase::B_RUNNING ||
             phase == TrackerAntennaPhase::SWAP_LOCKED)
      phase = resultA.valid ? TrackerAntennaPhase::A_SAVED
                            : TrackerAntennaPhase::IDLE;
    txLocked.store(false);
  }

  initialized.store(true);
  trackerDiagLog("ANT_BOOT", "phase=%s txLock=%u A=%u B=%u",
                 trackerAntennaPhaseText(phase), txLocked.load() ? 1U : 0U,
                 resultA.valid ? 1U : 0U, resultB.valid ? 1U : 0U);
}
...
bool trackerAntennaTxSafeToSwap() {
  if (!trackerAntennaTxLocked())
    return false;
  return !RadioLibInterface::instance ||
...
bool trackerAntennaHandleAction() {
  ...
  case TrackerAntennaPhase::COMPLETE: {
    ...
    trackerDiagLog("ANT_A_START", "ref=%s !%08x target=%u minimum=%u", name,
  ...
  case TrackerAntennaPhase::A_RUNNING: {
    ...
    if (!r.valid) {
      trackerDiagLog("ANT_A_WAIT", "samples=%u minimum=%u", (unsigned)r.samples,
                     (unsigned)ANT_MIN_SAMPLES);
      return true;
    }
    resultA = r;
    ...
    saveState();
    trackerDiagLog("ANT_A_SAVED",
                   "ref=!%08x n=%u rssi=%ddBm snr=%+.2fdB; TX remains normal",
                   (unsigned)referenceNode, (unsigned)resultA.samples,
                   (int)resultA.medianRssiDbm, resultA.medianSnrQ4 / 4.0f);
    clearLive();
    return true;
  }
  ...
  case TrackerAntennaPhase::A_SAVED: {
    ...
    if (!saveState()) {
      txLocked.store(false);
      phase = TrackerAntennaPhase::A_SAVED;
      trackerDiagLog("ANT_LOCK",
                     "PREP SWAP rejected: failed to persist TX lock");
      return true;
    }
    trackerDiagLog(
        "ANT_SWAP_LOCK",
        "TX locked; KEEP ANT CONNECTED until any in-flight TX finishes");
    return true;
  }
  ...
  case TrackerAntennaPhase::SWAP_LOCKED:
    if (!trackerAntennaTxSafeToSwap()) {
      trackerDiagLog(
          "ANT_SWAP_WAIT",
          "TX lock active but RF TX still in flight; keep antenna connected");
      return true;
    }
    ...
    if (!saveState()) {
      ...
      txLocked.store(true);
      phase = TrackerAntennaPhase::SWAP_LOCKED;
      trackerDiagLog("ANT_UNLOCK", "confirmation not accepted: state "
                                   "persistence failed; TX remains locked");
      return true;
    }
    trackerDiagLog("ANT_B_START",
                   "antenna connected confirmed; TX unlocked; ref=!%08x "
                   "target=%u minimum=%u",
                   (unsigned)referenceNode, (unsigned)ANT_TARGET_SAMPLES,
                   (unsigned)ANT_MIN_SAMPLES);
    return true;
  ...
  case TrackerAntennaPhase::B_RUNNING: {
    const TrackerAntennaResult r = finalizeLive();
    if (!r.valid) {
      trackerDiagLog("ANT_B_WAIT", "samples=%u minimum=%u", (unsigned)r.samples,
                     (unsigned)ANT_MIN_SAMPLES);
      return true;
    }
    resultB = r;
    ...
    trackerDiagLog("ANT_RESULT",
                   "ref=!%08x A=%ddBm/%+.2fdB B=%ddBm/%+.2fdB deltaRSSI=%+ddB "
                   "deltaSNR=%+.2fdB",
                   (unsigned)referenceNode, (int)resultA.medianRssiDbm,
                   resultA.medianSnrQ4 / 4.0f, (int)resultB.medianRssiDbm,
                   resultB.medianSnrQ4 / 4.0f, deltaRssi, deltaSnrQ4 / 4.0f);
    clearLive();
    return true;
  }
  }
