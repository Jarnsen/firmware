...
void trackerAntennaTestInit()
{
    ...
    phase =
        storedPhase <= (uint8_t)TrackerAntennaPhase::SWAP_LOCKED ? (TrackerAntennaPhase)storedPhase : TrackerAntennaPhase::IDLE;
    ...
    initialized.store(true);
    trackerDiagLog("ANT_BOOT", "phase=%s txLock=%u A=%u B=%u", trackerAntennaPhaseText(phase), txLocked.load() ? 1U : 0U,
...
bool trackerAntennaHandleAction()
{
    ...
    case TrackerAntennaPhase::COMPLETE: {
        ...
        trackerDiagLog("ANT_A_START", "ref=%s !%08x target=%u minimum=%u", name, (unsigned)ref, (unsigned)ANT_TARGET_SAMPLES,
    ...
    case TrackerAntennaPhase::A_RUNNING: {
        ...
        saveState();
        trackerDiagLog("ANT_A_SAVED", "ref=!%08x n=%u rssi=%ddBm snr=%+.2fdB; TX remains normal", (unsigned)referenceNode,
                       (unsigned)resultA.samples, (int)resultA.medianRssiDbm, resultA.medianSnrQ4 / 4.0f);
        clearLive();
        return true;
    }
