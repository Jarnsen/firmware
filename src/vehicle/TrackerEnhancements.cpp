...
static bool samePosition(const meshtastic_PositionLite &a,
                         const meshtastic_PositionLite &b) {
  return a.latitude_i == b.latitude_i && a.longitude_i == b.longitude_i &&
         a.time == b.time;
}

static uint32_t distanceMeters(const meshtastic_PositionLite &a,
                               const meshtastic_PositionLite &b) {
...
static void learnTtff(uint32_t sampleMs) {
  ...
  LOG_INFO("Tracker V1.1 diagnostics: TTFF sample=%ums learned=%ums",
           (unsigned)sampleMs, (unsigned)learnedTtffMs);
}
  ...
  if (bootWakeCause == ESP_SLEEP_WAKEUP_TIMER) {
    if (baselinePositionValid && !motionWakeSinceBaseline) {
      ...
      if (moved >= TRACKER_SENSOR_DIAG_MOVE_METERS) {
        ...
        LOG_WARN("Tracker V1.1 sensor self-check: moved %um without GPIO7 "
                 "wake; inspect SW-18010P/wiring",
                 (unsigned)moved);
      }
    }
...
static void updateSensorPinHealth(uint32_t now) {
  ...
  if (digitalRead(VEHICLE_MOTION_WAKE_PIN) == LOW) {
    ...
    else if ((uint32_t)(now - sensorLowStartedMs) >=
             TRACKER_SENSOR_STUCK_LOW_MS)
      motionSensorSuspect = true;
  } else {
...
protected:
  int32_t runOnce() override {
    ...
    if (readFreshPosition(position) &&
        (!lastObservedPositionValid ||
         !samePosition(position, lastObservedPosition))) {
...
void setupTrackerEnhancements() {
  ...
  LOG_INFO(
      "Tracker V1.1 enhancements: boot=%u wake=%s learnedTTFF=%ums sensor=%s",
      (unsigned)enhancementBootCount, trackerBootWakeReason(),
      (unsigned)learnedTtffMs, trackerMotionSensorStatus());
}

uint32_t trackerLearnedTtffMs() { return learnedTtffMs; }
...
uint32_t trackerEnhancementBootCount() { return enhancementBootCount; }

bool trackerMotionSensorSuspect() { return motionSensorSuspect; }
