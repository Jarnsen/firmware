...
static void IRAM_ATTR takLeaderMotionISR() { leaderMotionEdgeSequence++; }

static void drawTakLeaderServiceFrame(OLEDDisplay *display,
...
static void buildTakLeaderServicePage() {
  ...
  case TAK_PAGE_STATUS:
    snprintf(leaderBanner, sizeof(leaderBanner),
             "TAK SERVICE\nBAT %u%% GPS %s\nSHORT = NEXT", battery,
  ...
  case TAK_PAGE_DIAG:
    snprintf(leaderBanner, sizeof(leaderBanner),
             "DIAG GPS %s\nFIX %us HB %us\nSENSOR %s",
             positionKnown ? "FIX" : "WAIT",
             takLeaderDisplayAge(trackerLastFixAgeSecs()),
             takLeaderHeartbeatAgeSecs(), trackerMotionSensorStatus());
    break;
  case TAK_PAGE_VERSION:
    snprintf(leaderBanner, sizeof(leaderBanner), "%s\nBUILD %.8s\nUP %umin %s",
             JARNSEN_FIRMWARE_VERSION, JARNSEN_BUILD_SHA,
             (unsigned)(millis() / 60000UL), trackerBootWakeReason());
    break;
...
static void renderTakLeaderServicePage() {
  ...
  leaderDisplayWindowMs = takLeaderLowBattery()
                              ? TAK_LEADER_LOW_BATTERY_DISPLAY_MS
                              : TAK_LEADER_DISPLAY_MS;
...
static void startTakLeaderService() {
  ...
  renderTakLeaderServicePage();
  LOG_INFO("TAK leader: GPIO0 opened ATAK/Bluetooth/settings; %us idle, "
           "activity=%u/%us, %us hard cap",
...
static bool takLeaderServiceStillActive(uint32_t now) {
  if (!leaderServiceActive)
    return false;
  return (uint32_t)(now - leaderServiceStartedMs) <
             (uint32_t)TAK_LEADER_SERVICE_MAX_MS &&
         (uint32_t)(now - leaderServiceLastActivityMs) <
             (uint32_t)TAK_LEADER_SERVICE_MS;
}
  ...
  if (low) {
    if (leaderMotionPinLowSinceMs == 0) {
      leaderMotionPinLowSinceMs = now ? now : 1;
               ...
               (uint32_t)(now - leaderMotionPinLowSinceMs) >=
                   TAK_LEADER_MOTION_STUCK_LOW_MS) {
      leaderMotionWakeDisabledForStuckLow = true;
      LOG_WARN("TAK leader: GPIO%d LOW for %us; light-sleep motion wake "
               "temporarily disabled",
               VEHICLE_MOTION_WAKE_PIN,
               (unsigned)(TAK_LEADER_MOTION_STUCK_LOW_MS / 1000UL));
    }
  } else {
    leaderMotionPinLowSinceMs = 0;
    if (leaderMotionWakeDisabledForStuckLow) {
      leaderMotionWakeDisabledForStuckLow = false;
      LOG_INFO("TAK leader: GPIO%d recovered HIGH; light-sleep motion wake "
               "available",
               VEHICLE_MOTION_WAKE_PIN);
    }
  }
...
static void confirmTakLeaderMotion(uint32_t now) {
  ...
  LOG_INFO("TAK leader: movement confirmed (%u pulses within %ums)",
           (unsigned)trackerMotionConfirmCount(),
...
static void finishTakLeaderMotionWithPosition(uint32_t now) {
  if (!leaderMotionActive ||
      (uint32_t)(now - leaderLastMotionMs) < TAK_LEADER_MOTION_QUIET_MS)
    return;
  ...
  if (takLeaderHasFreshPosition()) {
    ...
    LOG_INFO("TAK leader: 120s motion quiet; fresh final position sent, "
             "returning to light sleep");
    return;
  }

  if (leaderFinalPositionWaitStartedMs == 0) {
    leaderFinalPositionWaitStartedMs = now ? now : 1;
    LOG_INFO("TAK leader: 120s motion quiet; waiting up to %us for fresh final "
             "GNSS fix",
             (unsigned)(TAK_LEADER_FINAL_GPS_WAIT_MS / 1000UL));
    return;
  }
  if ((uint32_t)(now - leaderFinalPositionWaitStartedMs) <
      TAK_LEADER_FINAL_GPS_WAIT_MS)
    return;

  if (positionModule && nodeDB && nodeDB->hasLocalPositionSinceBoot()) {
    positionModule->sendOurPosition();
    markTakLeaderPositionSent();
    LOG_WARN("TAK leader: final GNSS wait expired; sent best available "
             "position before light sleep");
  } else {
...
static void processTakLeaderMotion(uint32_t now) {
  ...
  if (newEdges != 0) {
    LOG_DEBUG("TAK motion: GPIO%d +%u edge(s), candidate=%u/%u active=%u",
              VEHICLE_MOTION_WAKE_PIN, (unsigned)newEdges,
              (unsigned)leaderMotionCandidateCount,
              (unsigned)trackerMotionConfirmCount(),
    ...
    if (leaderMotionActive) {
      ...
      const uint32_t needed =
          confirmCount > leaderMotionCandidateCount
              ? (uint32_t)confirmCount - leaderMotionCandidateCount
              : 0U;
      const uint32_t accepted = newEdges < needed ? newEdges : needed;
      leaderMotionCandidateCount =
          (uint8_t)(leaderMotionCandidateCount + accepted);
  ...
  if (leaderMotionCandidatePending &&
      (uint32_t)(now - leaderMotionCandidateStartedMs) >=
          trackerMotionConfirmWindowMs()) {
    LOG_DEBUG("TAK leader: rejected vibration candidate (%u/%u pulses)",
              (unsigned)leaderMotionCandidateCount,
              (unsigned)trackerMotionConfirmCount());
    leaderMotionCandidateCount = 0;
    leaderMotionCandidateStartedMs = 0;
...
static void updateTakLeaderPositionHeartbeat() {
  ...
  if (nowEpoch >= leaderLastPositionHeartbeatEpoch &&
    ...
    positionModule->sendOurPosition();
    leaderLastPositionHeartbeatEpoch = nowEpoch;
    LOG_INFO("TAK leader: autonomous position heartbeat sent after %us",
             (unsigned)heartbeatSecs);
  }
}
  ...
  int onNotify(void *) override {
    ...
    if (!leaderMotionWakeDisabledForStuckLow &&
      ...
      if (err == ESP_OK)
        ...
        LOG_ERROR(
            "TAK leader: failed to arm GPIO%d light-sleep motion wake: %d",
...
class TakLeaderMotionLightSleepEndObserver
...
static TakLeaderMotionLightSleepBeginObserver
    takLeaderMotionLightSleepBeginObserver;
static TakLeaderMotionLightSleepEndObserver
    takLeaderMotionLightSleepEndObserver;
...
protected:
  int onNotify(void *deepSleepMarker) override {
    if (!takLeaderEnabled() || deepSleepMarker != nullptr)
      return 0;
    return (leaderServiceActive || leaderMotionActive ||
            leaderMotionCandidatePending)
               ? 1
               : 0;
  }
};
...
public:
  HeltecTrackerV11TakLeaderPolicyThread()
      : concurrency::OSThread("TakLeaderPolicy") {}
...
protected:
  int32_t runOnce() override {
    ...
    if (!leaderBootHandoffComplete && graphics::isBootScreenComplete()) {
      ...
      LOG_INFO("TAK leader: Meshtastic boot screen complete; custom display "
               "ownership active");
      if (leaderServiceActive)
        renderTakLeaderServicePage();
    ...
    if (button != GPIO_NUM_NC) {
      ...
      if (pressed) {
        ...
        if (!leaderButtonLatched) {
          ...
          } else if (!leaderBootHandoffComplete ||
                     !takLeaderDisplayWindowActive(now) ||
        ...
        if (leaderButtonLatched && leaderServiceActive &&
            !leaderOpenedServiceThisPress && !leaderLongPressHandled &&
            (uint32_t)(now - leaderButtonLowSinceMs) >=
                (uint32_t)TAK_LEADER_MENU_LONG_PRESS_MS) {
        ...
        if ((uint32_t)(now - leaderButtonHighSinceMs) >= 25U) {
          if (leaderServiceActive && !leaderOpenedServiceThisPress &&
              !leaderLongPressHandled) {
            leaderServicePage =
                (uint8_t)((leaderServicePage + 1U) % TAK_PAGE_COUNT);
            renderTakLeaderServicePage();
            LOG_DEBUG("TAK leader: GPIO0 short press -> page %u/%u",
                      (unsigned)(leaderServicePage + 1U),
                      (unsigned)TAK_PAGE_COUNT);
          }
    ...
    if (trafficNow < leaderBleTrafficLast) {
      ...
      if (leaderBleActivityWindowStartedMs == 0 ||
          (uint32_t)(now - leaderBleActivityWindowStartedMs) >
              TAK_LEADER_SERVICE_ACTIVITY_WINDOW_MS) {
      ...
      leaderBleActivityWindowCount =
          count > (uint32_t)TAK_LEADER_SERVICE_ACTIVITY_THRESHOLD
      ...
      if (leaderServiceActive &&
        ...
        leaderServiceLastActivityMs = now;
        LOG_DEBUG(
            "TAK leader: active BLE burst detected; 120s idle timer reset");
        leaderBleActivityWindowStartedMs = now ? now : 1;
        leaderBleActivityWindowCount = 0;
      }
    }
...
void setupHeltecTrackerV11TakLeaderPolicy() {
  ...
  if (UserButtonThread) {
    ...
    LOG_INFO("TAK leader: generic Meshtastic UserButton disabled; GPIO0 "
             "exclusively owned by TAK service");
  }
#endif
  ...
  LOG_INFO("TAK leader profile: GNSS + LoRa light sleep, Smart Position + "
           "final fix, jittered heartbeat, exclusive GPIO0 service");
