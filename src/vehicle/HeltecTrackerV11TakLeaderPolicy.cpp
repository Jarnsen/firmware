...
static void buildTakLeaderServicePage()
{
    ...
    case TAK_PAGE_VERSION:
        snprintf(leaderBanner, sizeof(leaderBanner), "%s\nBUILD %.8s\nUP %umin %s", JARNSEN_FIRMWARE_VERSION, JARNSEN_BUILD_SHA,
                 (unsigned)(millis() / 60000UL), trackerBootWakeReason());
        break;
    case TAK_PAGE_MOTION:
        snprintf(leaderBanner, sizeof(leaderBanner), "MOTION %s\n%u PULSES / %us\nLONG = CHANGE", trackerMotionSensitivityName(),
                 (unsigned)trackerMotionConfirmCount(), (unsigned)(trackerMotionConfirmWindowMs() / 1000UL));
        break;
...
static void startTakLeaderService()
{
    ...
    LOG_INFO("TAK leader: GPIO0 opened ATAK/Bluetooth/settings; %us idle, "
             ...
             (unsigned)(TAK_LEADER_SERVICE_MS / 1000UL), (unsigned)TAK_LEADER_SERVICE_ACTIVITY_THRESHOLD,
             (unsigned)(TAK_LEADER_SERVICE_ACTIVITY_WINDOW_MS / 1000UL), (unsigned)(TAK_LEADER_SERVICE_MAX_MS / 1000UL));
}
...
static void processTakLeaderMotion(uint32_t now)
{
    ...
    if (newEdges != 0) {
        LOG_DEBUG("TAK motion: GPIO%d +%u edge(s), candidate=%u/%u active=%u", VEHICLE_MOTION_WAKE_PIN, (unsigned)newEdges,
                  (unsigned)leaderMotionCandidateCount, (unsigned)trackerMotionConfirmCount(), leaderMotionActive ? 1U : 0U);
        if (leaderMotionActive) {
            ...
            const uint32_t needed =
                confirmCount > leaderMotionCandidateCount ? (uint32_t)confirmCount - leaderMotionCandidateCount : 0U;
    ...
    if (leaderMotionCandidatePending && (uint32_t)(now - leaderMotionCandidateStartedMs) >= trackerMotionConfirmWindowMs()) {
...
static void updateTakLeaderPositionHeartbeat()
{
    ...
    if (nowEpoch >= leaderLastPositionHeartbeatEpoch && (nowEpoch - leaderLastPositionHeartbeatEpoch) >= heartbeatSecs) {
        positionModule->sendOurPosition();
        leaderLastPositionHeartbeatEpoch = nowEpoch;
    ...
    int onNotify(void *) override
    {
        ...
        if (!leaderMotionWakeDisabledForStuckLow && digitalRead(VEHICLE_MOTION_WAKE_PIN) != LOW) {
            ...
            if (err == ESP_OK)
                ...
                LOG_ERROR("TAK leader: failed to arm GPIO%d light-sleep motion wake: %d", VEHICLE_MOTION_WAKE_PIN, (int)err);
        }
        return 0;
    }
};
...
class TakLeaderMotionLightSleepEndObserver : public Observer<esp_sleep_wakeup_cause_t>
{
  protected:
    int onNotify(esp_sleep_wakeup_cause_t cause) override
    {
        ...
        if (cause == ESP_SLEEP_WAKEUP_GPIO && leaderMotionLightSleepWakeArmed && digitalRead(VEHICLE_MOTION_WAKE_PIN) == LOW)
            leaderMotionEdgeSequence++;
  ...
  protected:
    int32_t runOnce() override
    {
            ...
            if (pressed) {
                ...
                if ((uint32_t)(now - leaderButtonHighSinceMs) >= 25U) {
                    if (leaderServiceActive && !leaderOpenedServiceThisPress && !leaderLongPressHandled) {
                        ...
                        renderTakLeaderServicePage();
                        LOG_DEBUG("TAK leader: GPIO0 short press -> page %u/%u", (unsigned)(leaderServicePage + 1U),
        ...
        if (trafficNow < leaderBleTrafficLast) {
            ...
            if (leaderServiceActive && leaderBleActivityWindowCount >= (uint8_t)TAK_LEADER_SERVICE_ACTIVITY_THRESHOLD) {
                leaderServiceLastActivityMs = now;
