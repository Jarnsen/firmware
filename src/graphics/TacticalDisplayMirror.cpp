...
bool colorMonoMatchesDisplay(const uint8_t *displayBuffer, size_t monoBytes) {
  ...
  for (size_t i = 0; i < monoBytes; ++i) {
    differentBits += static_cast<size_t>(__builtin_popcount(
        static_cast<unsigned>(colorPublishedMono[i] ^ displayBuffer[i])));
...
size_t countColorRuns(const std::vector<uint16_t> &pixels) {
  ...
  for (size_t offset = 0; offset < pixels.size();) {
    ...
    size_t count = 1;
    while (offset + count < pixels.size() && pixels[offset + count] == color &&
           count < 255)
      ++count;
    offset += count;
    ++runs;
...
bool emitChunk(char mode, uint16_t width, uint16_t height, uint32_t sequence,
               uint16_t chunkIndex, uint16_t chunkCount, const uint8_t *payload,
               size_t payloadSize) {
  static constexpr char HEX_DIGITS[] = "0123456789ABCDEF";
  char line[MIRROR_LINE_BUFFER_SIZE];
  const int headerLength = snprintf(
      line, sizeof(line), "@TMF3 %c %u %u %lu %u %u ", mode,
      static_cast<unsigned>(width), static_cast<unsigned>(height),
      static_cast<unsigned long>(sequence), static_cast<unsigned>(chunkIndex),
  ...
  if (Serial.availableForWrite() < static_cast<int>(lineLength))
    return false;
  return Serial.write(reinterpret_cast<const uint8_t *>(line), lineLength) ==
         lineLength;
}
...
bool startPendingColorFrame(OLEDDisplay *display) {
  ...
  if (colorSequence == emittedColorSequence || colorPublished.empty() ||
      colorPublishedMono.empty() || !colorPublishedAt ||
      static_cast<uint32_t>(now - colorPublishedAt) > COLOR_FRAME_VALID_MS ||
  ...
  const size_t chunkCount =
      (runCount + COLOR_RUNS_PER_CHUNK - 1) / COLOR_RUNS_PER_CHUNK;
...
bool startPendingMonoFrame(OLEDDisplay *display) {
  ...
  const bool sameGeometry = havePreviousMono && previousWidth == width &&
                            previousHeight == height &&
                            previousMono.size() == frameBytes;
  if (sameGeometry &&
      memcmp(previousMono.data(), display->buffer, frameBytes) == 0)
    return false;

  pendingMono.assign(display->buffer, display->buffer + frameBytes);
  const size_t chunkCount =
      (frameBytes + CHUNK_PAYLOAD_BYTES - 1) / CHUNK_PAYLOAD_BYTES;
  if (chunkCount > UINT16_MAX) {
    pendingMono.clear();
    return false;
  }
...
bool emitPendingColorChunk() {
  ...
  size_t nextOffset = pendingFrame.sourceOffset;
  {
    ...
    while (nextOffset < colorPublished.size() && runs < COLOR_RUNS_PER_CHUNK) {
      ...
      uint8_t count = 1;
      while (nextOffset + count < colorPublished.size() &&
             colorPublished[nextOffset + count] == color && count < 255)
        ++count;
  ...
  if (!payloadSize || !emitChunk('C', pendingFrame.width, pendingFrame.height,
                                 pendingFrame.sequence, pendingFrame.chunkIndex,
...
bool emitPendingMonoChunk() {
  ...
  if (!payloadSize ||
      !emitChunk('M', pendingFrame.width, pendingFrame.height,
                 pendingFrame.sequence, pendingFrame.chunkIndex,
                 pendingFrame.chunkCount,
                 pendingMono.data() + pendingFrame.sourceOffset, payloadSize))
    return false;
...
void beginTacticalColorFrame(uint16_t width, uint16_t height,
                             uint16_t backgroundRgb565) {
  if (!width || !height)
    return;
  concurrency::LockGuard guard(&colorLock);
  colorWidth = width;
...
void drawTacticalColorLine(int16_t x1, int16_t y1, int16_t x2, int16_t y2,
                           uint16_t rgb565) {
  concurrency::LockGuard guard(&colorLock);
  if (!colorBuilding)
    return;
...
void drawTacticalColorCircle(int16_t centerX, int16_t centerY, int16_t radius,
                             uint16_t rgb565) {
  if (radius < 0)
    return;
  concurrency::LockGuard guard(&colorLock);
...
void overlayTacticalMonoBuffer(OLEDDisplay *display, uint16_t foregroundRgb565,
                               uint16_t onlyOverRgb565) {
  if (!display || !display->buffer)
    return;
  concurrency::LockGuard guard(&colorLock);
  if (!colorBuilding || display->getWidth() != colorWidth ||
  ...
  for (uint16_t y = 0; y < colorHeight; ++y) {
    ...
    for (uint16_t x = 0; x < colorWidth; ++x) {
      const size_t pixelIndex = static_cast<size_t>(y) * colorWidth + x;
      if ((display->buffer[pageOffset + x] & mask) &&
          colorWorking[pixelIndex] == onlyOverRgb565)
...
void publishTacticalColorFrame() {
  ...
  if (colorPublished == colorWorking &&
...
bool getTacticalColorFrameInfo(const uint8_t *monoBuffer, size_t monoBytes,
                               uint16_t &width, uint16_t &height,
                               uint32_t &sequence) {
  concurrency::LockGuard guard(&colorLock);
  const uint32_t now = millis();
  if (!monoBuffer || colorPublished.empty() || !colorWidth || !colorHeight ||
      !colorSequence || !colorPublishedAt ||
      static_cast<uint32_t>(now - colorPublishedAt) > COLOR_FRAME_VALID_MS ||
      !colorMonoMatchesDisplay(monoBuffer, monoBytes))
    return false;
...
bool copyTacticalColorFrameRows(uint32_t sequence, uint16_t startRow,
                                uint16_t rowCount, uint16_t *destination,
                                size_t destinationPixels) {
  if (!destination || !rowCount)
    return false;
  concurrency::LockGuard guard(&colorLock);
  if (sequence != colorSequence || colorPublished.empty() ||
      startRow >= colorHeight || rowCount > colorHeight - startRow)
    return false;
  ...
  if (destinationPixels < pixelCount)
    return false;
  memcpy(destination,
         colorPublished.data() + static_cast<size_t>(startRow) * colorWidth,
