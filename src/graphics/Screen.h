...
private:
  ...
  void drawFrame(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x,
                 int16_t y);
  void drawFrameSettings(OLEDDisplay *display, OLEDDisplayUiState *state,
                         int16_t x, int16_t y);
  void drawFrameWiFi(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x,
                     int16_t y);
...
class Screen : public concurrency::OSThread {
  CallbackObserver<Screen, const meshtastic::Status *> powerStatusObserver =
      CallbackObserver<Screen, const meshtastic::Status *>(
          this, &Screen::handleStatusUpdate);
  ...
  CallbackObserver<Screen, const UIFrameEvent *> uiFrameEventObserver =
      CallbackObserver<Screen, const UIFrameEvent *>(
          this, &Screen::handleUIFrameEvent); // Sent by Mesh Modules
  CallbackObserver<Screen, const InputEvent *> inputObserver =
      CallbackObserver<Screen, const InputEvent *>(this,
                                                   &Screen::handleInputEvent);
  CallbackObserver<Screen, AdminModule_ObserverData *> adminMessageObserver =
      CallbackObserver<Screen, AdminModule_ObserverData *>(
          this, &Screen::handleAdminMessage);
...
public:
  ...
  uint8_t currentFrameIndex() {
    return ui ? ui->getUiState()->currentFrame : 255;
  }
  ...
  void startAlert(const char *_alertMessage) {
    startAlert([_alertMessage](OLEDDisplay *display, OLEDDisplayUiState *state,
  ...
  void showNodePicker(const char *message, uint32_t durationMs,
                      std::function<void(uint32_t)> bannerCallback);
  void showNumberPicker(const char *message, uint32_t durationMs,
  ...
  void showTextInput(const char *header, const char *initialText,
                     uint32_t durationMs,
  ...
  void setEndCalibration(uint32_t _endCalibrationAt) {
    endCalibrationAt = _endCalibrationAt;
  }
...
private:
  ...
  uint8_t brightness =
      BRIGHTNESS_DEFAULT; // H = 254, MH = 192, ML = 130 L = 103
