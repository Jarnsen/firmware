...
// DEBUG
#if BASEUI_HAS_GAMES
#define NUM_EXTRA_FRAMES                                                       \
  4 // text message, debug frame, and the always-present games frame
#else
...
bool isBootScreenComplete() { return bootScreenComplete; }
...
static inline void prepareFrameColorRegions() {
  ...
  if (isThemeFullFrameInvert()) {
    setAndRegisterTFTColorRole(TFTColorRole::FrameMono, getThemeBodyFg(),
                               getThemeBodyBg(), 0, 0, screen->getWidth(),
                               screen->getHeight());
  }
#endif
}
...
static inline void updateUiFrame(OLEDDisplayUi *ui) {
  ...
  if (meshtastic_security::shouldRedactDisplay() && screen != nullptr) {
    ...
    if (NotificationRenderer::current_notification_type ==
        notificationTypeEnum::pairing_pin) {
...
void Screen::showOverlayBanner(BannerOverlayOptions banner_overlay_options) {
  ...
  strncpy(NotificationRenderer::alertBannerMessage,
          banner_overlay_options.message, 255);
  NotificationRenderer::parseBannerMessageWithFonts(
      NotificationRenderer::alertBannerMessage);
  NotificationRenderer::alertBannerMessage[255] =
      '\0'; // Ensure null termination
  NotificationRenderer::alertBannerUntil =
      (banner_overlay_options.durationMs == 0)
          ? 0
          : millis() + banner_overlay_options.durationMs;
  NotificationRenderer::optionsArrayPtr =
      banner_overlay_options.optionsArrayPtr;
  NotificationRenderer::optionsEnumPtr = banner_overlay_options.optionsEnumPtr;
  NotificationRenderer::alertBannerOptions =
      banner_overlay_options.optionsCount;
  NotificationRenderer::alertBannerCallback =
      banner_overlay_options.bannerCallback;
  ...
  NotificationRenderer::current_notification_type =
      banner_overlay_options.notificationType;
...
// Called to trigger a banner with custom message and duration
void Screen::showNodePicker(const char *message, uint32_t durationMs,
  ...
  NotificationRenderer::alertBannerMessage[255] =
      '\0'; // Ensure null termination
  NotificationRenderer::alertBannerUntil =
      (durationMs == 0) ? 0 : millis() + durationMs;
  NotificationRenderer::alertBannerCallback = bannerCallback;
  NotificationRenderer::pauseBanner = false;
  NotificationRenderer::curSelected = 0;
  NotificationRenderer::current_notification_type =
      notificationTypeEnum::node_picker;
...
// Called to trigger a banner with custom message and duration
void Screen::showNumberPicker(const char *message, uint32_t durationMs,
                              uint8_t digits, bool useBase16,
                              std::function<void(uint32_t)> bannerCallback) {
#ifdef USE_EINK
  EINK_ADD_FRAMEFLAG(dispdev,
                     DEMAND_FAST); // Skip full refresh for all overlay menus
  ...
  if (useBase16)
    NotificationRenderer::current_notification_type =
        notificationTypeEnum::hex_picker;
  else
    NotificationRenderer::current_notification_type =
        notificationTypeEnum::number_picker;
...
// the sibling flow).
void Screen::showAlphanumericPicker(
    const char *message, const char *initialText, uint32_t durationMs,
    uint8_t length, std::function<void(const std::string &)> bannerCallback) {
#ifdef USE_EINK
  EINK_ADD_FRAMEFLAG(dispdev,
                     DEMAND_FAST); // Skip full refresh for all overlay menus
#endif
...
void Screen::showTextInput(
    const char *header, const char *initialText, uint32_t durationMs,
    std::function<void(const std::string &)> textCallback) {
  LOG_INFO("showTextInput called with header='%s', durationMs=%d",
           header ? header : "NULL", durationMs);

  // Start OnScreenKeyboardModule session (non-touch variant)
  OnScreenKeyboardModule::instance().start(header, initialText, durationMs,
                                           textCallback);
  ...
  // other notifications)
  strncpy(NotificationRenderer::alertBannerMessage,
          header ? header : "Text Input", 255);
  NotificationRenderer::alertBannerMessage[255] = '\0';
  NotificationRenderer::alertBannerUntil =
      (durationMs == 0) ? 0 : millis() + durationMs;
  NotificationRenderer::pauseBanner = false;
  NotificationRenderer::current_notification_type =
      notificationTypeEnum::text_input;
...
static void drawModuleFrame(OLEDDisplay *display, OLEDDisplayUiState *state,
                            int16_t x, int16_t y) {
...
// drawModuleFrame.
static void drawGamesFrame(OLEDDisplay *display, OLEDDisplayUiState *state,
                           int16_t x, int16_t y) {
  if (gamesModule)
...
float Screen::estimatedHeading(double lat, double lon) {
  ...
  const uint32_t gpsUpdateIntervalSecs = Default::getConfiguredOrDefault(
  ...
  const uint32_t headingStaleMs =
      (effectiveUpdateIntervalSecs > (UINT32_MAX / 2000U))
          ? UINT32_MAX
          : (effectiveUpdateIntervalSecs * 2000U);
...
Screen::Screen(ScanI2C::DeviceAddress address,
    ...
    : concurrency::OSThread("Screen"), address_found(address),
      model(screenType), geometry(geometry), cmdQueue(32) {
  graphics::normalFrames = new FrameCallback[MAX_NUM_NODES + NUM_EXTRA_FRAMES];

#if defined(USE_SH1106) || defined(USE_SH1107) || defined(USE_SH1107_128_64)
  dispdev = new SH1106Wire(address.address, -1, -1, geometry,
                           (address.port == ScanI2C::I2CPort::WIRE1)
                               ? HW_I2C::I2C_TWO
                               : HW_I2C::I2C_ONE);
  isI2cScreen = true;
#elif defined(USE_ST7789)
#ifdef ESP_PLATFORM
  dispdev = new ST7789Spi(&SPI1, ST7789_RESET, ST7789_RS, ST7789_NSS,
                          GEOMETRY_RAWMODE, TFT_WIDTH, TFT_HEIGHT, ST7789_SDA,
                          ST7789_MISO, ST7789_SCK);
#else
  dispdev = new ST7789Spi(&SPI1, ST7789_RESET, ST7789_RS, ST7789_NSS,
                          GEOMETRY_RAWMODE, TFT_WIDTH, TFT_HEIGHT);
#endif
#elif defined(USE_ST7796)
#ifdef ESP_PLATFORM
  dispdev = new ST7796Spi(&SPI1, ST7796_RESET, ST7796_RS, ST7796_NSS,
                          GEOMETRY_RAWMODE, TFT_WIDTH, TFT_HEIGHT, ST7796_SDA,
  ...
  dispdev = new HUB75Display(address.address, -1, -1, GEOMETRY_RAWMODE,
                             HW_I2C::I2C_ONE);
  ...
  dispdev =
      new SSD1306Spi(SSD1306_RESET, SSD1306_RS, SSD1306_NSS, GEOMETRY_64_48);
  if (!dispdev->init()) {
  ...
  if (portduino_config.displayPanel == hub75) {
    ...
    dispdev = new HUB75Native(address.address, -1, -1, GEOMETRY_RAWMODE,
                              HW_I2C::I2C_ONE);
#else
  ...
  dispdev = new TFTDisplay(address.address, -1, -1, geometry,
    ...
    !defined(USE_EINK_PARALLELDISPLAY)
  ...
  dispdev = new EInkParallelDisplay(EPD_WIDTH, EPD_HEIGHT,
                                    EInkParallelDisplay::EPD_ROT_PORTRAIT);
...
void Screen::handleSetOn(bool on, FrameCallback einkScreensaver) {
  ...
  if (meshtasticTrackerScreenPowerAllowed &&
      !meshtasticTrackerScreenPowerAllowed(on))
    return;
...
// some screens seem to need a kick in the pants to turn back on
    ...
    !defined(M5STACK) // set display brightness when turning on screens. Just
...
void Screen::setup() {
  ...
  if (uiconfig.screen_brightness == 0) {
#if defined(USE_OLED) || defined(USE_SSD1306) || defined(USE_SH1106) ||        \
    defined(USE_SH1107)
  ...
  ui->getUiState()->userData =
      this; // Allow static callbacks to access Screen instance

  // Apply loaded brightness
#if defined(ST7789_CS)
  static_cast<TFTDisplay *>(dispdev)->setDisplayBrightness(brightness);
#elif defined(USE_OLED) || defined(USE_SSD1306) || defined(USE_SH1106) ||      \
    defined(USE_SH1107) || defined(USE_SPISSD1306)
  dispdev->setBrightness(brightness);
#endif
  ...
  alertFrames[0] = [this](OLEDDisplay *display, OLEDDisplayUiState *state,
                          int16_t x, int16_t y) {
#ifdef ARCH_ESP32
  ...
  handleSetOn(true);
  graphics::currentResolution =
      graphics::determineScreenResolution(dispdev->height(), dispdev->width());
  updateUiFrame(ui);
#ifndef USE_EINK
  ...
  if (config.display.displaymode !=
    ...
    if (portduino_config.touchscreenModule) {
      touchScreenImpl1 =
          new TouchScreenImpl1(dispdev->getWidth(), dispdev->getHeight(),
...
int32_t Screen::runOnce() {
  ...
  if (showingOEMBootScreen &&
      (millis() > ((logo_timeout / 2) + serialSinceMsec))) {
    LOG_INFO("Switch to OEM screen...");
    // Change frames.
    static FrameCallback bootOEMFrames[] = {
        graphics::UIRenderer::drawOEMBootScreen};
    static const int bootOEMFrameCount =
        sizeof(bootOEMFrames) / sizeof(bootOEMFrames[0]);
...
#endif
  if (!suppressRegionOnboard &&
      !NotificationRenderer::isOverlayBannerShowing() &&
  ...
  for (;;) {
    ...
    case Cmd::START_ALERT_FRAME: {
      showingBootScreen =
          false; // this should avoid the edge case where an alert triggers
  ...
  if (showingNormalScreen && hasCompass) {
    ...
    if ((framesetInfo.positions.gps != 255 &&
         currentFrame == framesetInfo.positions.gps) ||
        (framesetInfo.positions.waypoint != 255 &&
         currentFrame == framesetInfo.positions.waypoint) ||
        (framesetInfo.positions.firstFavorite != 255 &&
  ...
  if (targetFramerate != desiredFramerate &&
  ...
  if (showingNormalScreen) {
    ...
    if (config.display.auto_screen_carousel_secs > 0 &&
        NotificationRenderer::current_notification_type !=
            notificationTypeEnum::text_input &&
        !Throttle::isWithinTimespanMs(lastScreenTransition,
                                      config.display.auto_screen_carousel_secs *
                                          1000)) {
      ...
      LOG_DEBUG("LastScreenTransition exceeded %ums transition to next frame",
...
void Screen::setScreensaverFrames(FrameCallback einkScreensaver) {
  ...
  static_cast<EInkDisplay *>(dispdev)->forceDisplay(
      0); // Screen::forceDisplay(), but override rate-limit
#endif
  ...
  EINK_ADD_FRAMEFLAG(
      dispdev, COSMETIC); // Really ugly to see ghosting from "screen paused"
#else
  EINK_ADD_FRAMEFLAG(
      dispdev, RESPONSIVE); // Really nice to wake screen with a fast-refresh
#endif
}
#endif
...
void Screen::setFrames(FrameFocus focus) {
  ...
  if (bootScreenComplete && trackerOwnsScreenAfterBoot() &&
      !showingNormalScreen && focus != FOCUS_MODULE)
    return;
  ...
  if (!hiddenFrames.nodelist_nodes) {
    ...
    normalFrames[numframes++] =
        graphics::NodeListRenderer::drawDynamicListScreen_Nodes;
    indicatorIcons.push_back(icon_nodes);
  }
  if (!hiddenFrames.nodelist_location) {
    fsi.positions.nodelist_location = numframes;
    normalFrames[numframes++] =
        graphics::NodeListRenderer::drawDynamicListScreen_Location;
...
// Show detailed node views only on E-Ink builds
  ...
  if (!hiddenFrames.nodelist_bearings) {
    ...
    normalFrames[numframes++] =
        graphics::NodeListRenderer::drawNodeListWithCompasses;
    indicatorIcons.push_back(icon_list);
  }
#endif
  if (!hiddenFrames.gps) {
    ...
    normalFrames[numframes++] =
        graphics::UIRenderer::drawCompassAndLocationScreen;
  ...
  if (!hiddenFrames.wifi && isWifiAvailable()) {
    ...
    normalFrames[numframes++] =
        graphics::DebugRenderer::drawDebugInfoWiFiTrampoline;
  ...
  fsi.frameCount =
      numframes; // Total framecount is used to apply FOCUS_PRESERVE
...
void Screen::loadFrameVisibility() {
  ...
  if (file) {
    FrameVisFile data{};
    bool ok = file.read((uint8_t *)&data, sizeof(data)) == sizeof(data) &&
              data.magic == FRAMEVIS_MAGIC && data.version == FRAMEVIS_VERSION;
    file.close();
    spiLock->unlock();
    if (ok) {
...
void Screen::handleStartFirmwareUpdateScreen() {
  ...
  static FrameCallback frames[] = {
      graphics::NotificationRenderer::drawFrameFirmware};
...
void Screen::logFrameChange(const char *reason, uint8_t targetIdx) {
  ...
  if (targetIdx == p.home)
    ...
    name = "wifi";
  else if (p.firstFavorite != 255 && p.lastFavorite != 255 &&
           targetIdx >= p.firstFavorite && targetIdx <= p.lastFavorite)
    name = "favorite";
  LOG_INFO("Screen: frame %u/%u name=%s reason=%s", (unsigned)targetIdx,
           (unsigned)framesetInfo.frameCount, name, reason);
}
#endif
  ...
  if (ui->getUiState()->frameState == FIXED) {
    ...
    if (framesetInfo.frameCount > 0) {
      ...
      uint8_t target = (direction == FrameDirection::NEXT)
                           ...
                           : (uint8_t)((curr + framesetInfo.frameCount - 1) %
                                       framesetInfo.frameCount);
      logFrameChange(direction == FrameDirection::NEXT ? "next" : "prev",
                     target);
    }
#endif
...
int Screen::handleUIFrameEvent(const UIFrameEvent *event) {
  ...
  if (showingNormalScreen) {
    ...
    // current frame
             ...
             UIFrameEvent::Action::REGENERATE_FRAMESET_BACKGROUND) {
...
int Screen::handleInputEvent(const InputEvent *event) {
  ...
  EINK_ADD_FRAMEFLAG(
      dispdev, DEMAND_FAST); // Use fast-refresh for next frame, no skip please
  ...
  if (ui->getUiState()->currentFrame == framesetInfo.positions.nodelist_nodes ||
      ...
      ui->getUiState()->currentFrame ==
          framesetInfo.positions.nodelist_lastheard ||
      ui->getUiState()->currentFrame ==
          framesetInfo.positions.nodelist_hopsignal ||
      ui->getUiState()->currentFrame ==
          framesetInfo.positions.nodelist_distance ||
      ui->getUiState()->currentFrame ==
          framesetInfo.positions.nodelist_bearings) {
    if (event->inputEvent == INPUT_BROKER_UP) {
    ...
    if (!inputIntercepted) {
      ...
      const bool isTextMessageFrame =
          (framesetInfo.positions.textMessage != 255 &&
           this->ui->getUiState()->currentFrame ==
               framesetInfo.positions.textMessage &&
      ...
      if (event->inputEvent == INPUT_BROKER_LEFT ||
        ...
        showNextFrame();
                 ...
                 this->ui->getUiState()->currentFrame ==
                     framesetInfo.positions.home) {
        cannedMessageModule->LaunchWithDestination(NODENUM_BROADCAST);
      } else if (event->inputEvent == INPUT_BROKER_SELECT) {
        if (this->ui->getUiState()->currentFrame ==
            framesetInfo.positions.home) {
          menuHandler::homeBaseMenu();
#if BASEUI_HAS_GAMES
        } else if (gamesModule && framesetInfo.positions.games != 255 &&
                   this->ui->getUiState()->currentFrame ==
                       framesetInfo.positions.games) {
          gamesModule
              ->launchGame(); // launch the game shown on the attract screen
#endif
        } else if (this->ui->getUiState()->currentFrame ==
                   framesetInfo.positions.system) {
          menuHandler::systemBaseMenu();
#if HAS_GPS
        } else if (this->ui->getUiState()->currentFrame ==
                       framesetInfo.positions.gps &&
                   gps) {
          menuHandler::positionBaseMenu();
#endif
        } else if (this->ui->getUiState()->currentFrame ==
                   framesetInfo.positions.clock) {
          menuHandler::clockMenu();
        } else if (this->ui->getUiState()->currentFrame ==
                   framesetInfo.positions.lora) {
          ...
          if (messageStore.hasVisibleMessages()) {
            ...
            if (currentResolution == ScreenResolution::UltraLow) {
              ...
              menuHandler::textMessageBaseMenu();
                   ...
                   this->ui->getUiState()->currentFrame <=
                       framesetInfo.positions.lastFavorite) {
          menuHandler::favoriteBaseMenu();
        } else if (this->ui->getUiState()->currentFrame ==
                       framesetInfo.positions.nodelist_nodes ||
                   this->ui->getUiState()->currentFrame ==
                       framesetInfo.positions.nodelist_location ||
                   this->ui->getUiState()->currentFrame ==
                       framesetInfo.positions.nodelist_lastheard ||
...
bool Screen::isGamesFrameShown() {
  return framesetInfo.positions.games != 255 && ui &&
         ui->getUiState()->currentFrame == framesetInfo.positions.games;
}

} // namespace graphics

#else
graphics::Screen::Screen(ScanI2C::DeviceAddress,
