...
<div class="mapAttribution hide" id="mapAttribution">© OpenStreetMap</div>
...
let showNodes=true,showTrack=true,followSelf=true;
let view={lat:49.4,lon:7.0,span:.02};
const pointers=new Map();let dragStart=null,pinchStart=null;
const tileCache=new Map();let onlineMapState='checking',tileFailureStreak=0;
...
function setOnlineMapState(state){onlineMapState=state;const b=$('mapNetBadge'),a=$('mapAttribution');if(state==='online'){b.textContent='N ↑ · ONLINE';a.classList.remove('hide')}else if(state==='offline'){b.textContent='N ↑ · OFFLINE';a.classList.add('hide')}else{b.textContent='N ↑ · KARTE AUTO';a.classList.add('hide')}}
...
function trimTiles(){if(tileCache.size<=180)return;for(const [k,v] of tileCache){if(v.state!=='loading')tileCache.delete(k);if(tileCache.size<=140)break}}
function onlineTile(z,x,y){const n=2**z;if(y<0||y>=n)return null;const xn=((x%n)+n)%n,key=z+'/'+xn+'/'+y;let e=tileCache.get(key);if(e&&e.state==='error'&&Date.now()-e.at>30000){tileCache.delete(key);e=null}if(e)return e;trimTiles();const img=new Image();e={state:'loading',img,x:xn,y,z,at:Date.now()};tileCache.set(key,e);img.decoding='async';img.onload=()=>{e.state='ready';e.at=Date.now();tileFailureStreak=0;setOnlineMapState('online');drawMap()};img.onerror=()=>{e.state='error';e.at=Date.now();tileFailureStreak++;if(tileFailureStreak>=4)setOnlineMapState('offline')};img.src=TILE_BASE+z+'/'+xn+'/'+y+'.png';return e}
function drawOnlineTiles(ctx,w,h){const z=tileZoom(h),nw=unproject(0,0,w,h),se=unproject(w,h,w,h),west=Math.min(nw.lon,se.lon),east=Math.max(nw.lon,se.lon),north=Math.max(nw.lat,se.lat),south=Math.min(nw.lat,se.lat);let x0=Math.floor(tileX(west,z))-1,x1=Math.floor(tileX(east,z))+1,y0=Math.floor(tileY(north,z))-1,y1=Math.floor(tileY(south,z))+1;const count=(x1-x0+1)*(y1-y0+1);if(count<=0||count>64){setOnlineMapState('offline');return 0}let ready=0,pending=0;for(let y=y0;y<=y1;y++)for(let x=x0;x<=x1;x++){const e=onlineTile(z,x,y);if(!e)continue;if(e.state==='loading'){pending++;continue}if(e.state!=='ready')continue;let lonL=tileLon(e.x,z);while(lonL-view.lon>180)lonL-=360;while(view.lon-lonL>180)lonL+=360;const lonR=lonL+360/(2**z),latT=tileLat(y,z),latB=tileLat(y+1,z),a=project({lat:latT,lon:lonL},w,h),b=project({lat:latB,lon:lonR},w,h);ctx.drawImage(e.img,a.x,a.y,b.x-a.x,b.y-a.y);ready++}if(ready>0)setOnlineMapState('online');else if(pending>0&&onlineMapState!=='online')setOnlineMapState('checking');else if(tileFailureStreak>=4)setOnlineMapState('offline');return ready}
function drawGrid(ctx,w,h,t){ctx.fillStyle=t.map;ctx.fillRect(0,0,w,h);const online=drawOnlineTiles(ctx,w,h);ctx.save();ctx.globalAlpha=online?.22:1;ctx.strokeStyle=t.line;ctx.lineWidth=1;const stepPx=Math.max(54,Math.min(110,h/5));for(let x=(w/2)%stepPx;x<w;x+=stepPx){ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x,h);ctx.stroke()}for(let y=(h/2)%stepPx;y<h;y+=stepPx){ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(w,y);ctx.stroke()}ctx.restore()}
