...
static void buildStatus_LH(meshtastic_LockdownStatus &out,
                           meshtastic_LockdownStatus_State state,
                           const char *lock_reason, uint8_t boots_remaining,
  ...
  if (state == meshtastic_LockdownStatus_State_LOCKED && wireReason &&
      wireReason[0] != '\0') {
...
PhoneAPI::PhoneAPI() {
  ...
  std::fill(std::begin(recentToRadioPacketIds),
            std::end(recentToRadioPacketIds), 0);
}
...
void PhoneAPI::handleStartConfig() {
  ...
  if (config_nonce != SPECIAL_NONCE_ONLY_NODES) {
    bool filesManifestLimited = false;
    {
      ...
      filesManifest = getFiles("/", FILES_MANIFEST_LEVELS,
                               FILES_MANIFEST_MAX_COUNT, &filesManifestLimited);
    }
    if (filesManifestLimited) {
      LOG_WARN("Got %zu files in manifest (limited to %zu entries/depth %u)",
               filesManifest.size(), FILES_MANIFEST_MAX_COUNT,
...
bool PhoneAPI::handleToRadio(const uint8_t *buf, size_t bufLength) {
  powerFSM.trigger(
      EVENT_CONTACT_FROM_PHONE); // As long as the phone keeps talking to us,
                                 // don't let the radio go to sleep
  lastContactMsec = millis();

  memset(&toRadioScratch, 0, sizeof(toRadioScratch));
  if (pb_decode_from_bytes(buf, bufLength, &meshtastic_ToRadio_msg,
                           &toRadioScratch)) {
    ...
    case meshtastic_ToRadio_packet_tag:
      ...
      if (!getAdminAuthorized()) {
        ...
        bool isLocalAdmin = ourNum != 0 &&
                            toRadioScratch.packet.which_payload_variant ==
                                meshtastic_MeshPacket_decoded_tag &&
                            toRadioScratch.packet.decoded.portnum ==
                                meshtastic_PortNum_ADMIN_APP &&
                            toRadioScratch.packet.to == ourNum;
        if (!isLocalAdmin) {
      ...
      if (toRadioScratch.packet.which_payload_variant ==
              meshtastic_MeshPacket_decoded_tag &&
          toRadioScratch.packet.decoded.portnum ==
        ...
        if (pb_decode_from_bytes(toRadioScratch.packet.decoded.payload.bytes,
          ...
          LOG_WARN(
              "Heltec V3 phone GPS: malformed POSITION_APP payload ignored");
        }
      }
#endif
    ...
    case meshtastic_ToRadio_mqttClientProxyMessage_tag:
      ...
      if (state != STATE_SEND_PACKETS) {
        LOG_WARN(
            "Ignore MqttClientProxy message while completing config handshake");
        break;
      }
      if (mqtt && moduleConfig.mqtt.proxy_to_client_enabled &&
          moduleConfig.mqtt.enabled &&
          (channels.anyMqttEnabled() ||
...
size_t PhoneAPI::getFromRadio(uint8_t *buf) {
  ...
  if (heartbeatReceived) {
    ...
    heartbeatReceived = false;
    size_t numbytes =
        pb_encode_to_bytes(buf, meshtastic_FromRadio_size,
                           &meshtastic_FromRadio_msg, &fromRadioScratch);
  ...
  case STATE_SEND_MY_INFO:
    ...
    if (!getAdminAuthorized()) {
      ...
      memset(fromRadioScratch.my_info.pio_env, 0,
             sizeof(fromRadioScratch.my_info.pio_env));
  ...
  case STATE_SEND_REGION_PRESETS:
    ...
    fromRadioScratch.which_payload_variant =
        meshtastic_FromRadio_region_presets_tag;
  ...
  case STATE_SEND_MODULECONFIG:
    ...
    case meshtastic_ModuleConfig_mqtt_tag:
      ...
      fromRadioScratch.moduleConfig.which_payload_variant =
    ...
    case meshtastic_ModuleConfig_serial_tag:
      ...
      fromRadioScratch.moduleConfig.payload_variant.serial =
          moduleConfig.serial;
      break;
    case meshtastic_ModuleConfig_external_notification_tag:
      ...
      fromRadioScratch.moduleConfig.payload_variant.external_notification =
    ...
    case meshtastic_ModuleConfig_store_forward_tag:
      ...
      fromRadioScratch.moduleConfig.payload_variant.store_forward =
    ...
    case meshtastic_ModuleConfig_range_test_tag:
      ...
      fromRadioScratch.moduleConfig.payload_variant.range_test =
    ...
    case meshtastic_ModuleConfig_telemetry_tag:
      ...
      fromRadioScratch.moduleConfig.payload_variant.telemetry =
    ...
    case meshtastic_ModuleConfig_canned_message_tag:
      ...
      fromRadioScratch.moduleConfig.payload_variant.canned_message =
    ...
    case meshtastic_ModuleConfig_remote_hardware_tag:
      ...
      fromRadioScratch.moduleConfig.payload_variant.remote_hardware =
    ...
    case meshtastic_ModuleConfig_neighbor_info_tag:
      ...
      fromRadioScratch.moduleConfig.payload_variant.neighbor_info =
    ...
    case meshtastic_ModuleConfig_traffic_management_tag:
      ...
      fromRadioScratch.moduleConfig.payload_variant.traffic_management =
...
#endif
      {
        fromRadioScratch.moduleConfig.payload_variant.mesh_beacon =
            moduleConfig.mesh_beacon;
      }
      break;
#endif
    default:
  ...
  case STATE_SEND_OTHER_NODEINFOS: {
    if (readIndex ==
        2) { //  readIndex==2 will be true for the first non-us node
    ...
    if (infoToSend.num != 0) {
      ...
      LOG_DEBUG("Done sending %d of %d nodeinfos millis=%u", readIndex,
                nodeDB->getNumMeshNodes(), millis());
      nodeInfoMutex.lock();
      nodeInfoQueue.clear();
  ...
  case STATE_SEND_FILEMANIFEST: {
    ...
    if (config_state == filesManifest.size() ||
      ...
      LOG_DEBUG("File: %s (%d) bytes", fromRadioScratch.fileInfo.file_name,
                fromRadioScratch.fileInfo.size_bytes);
      config_state++;
    }
    break;
  }
  ...
  case STATE_SEND_PACKETS:
    ...
    if (queueStatusPacketForPhone) {
...
#endif
      {
        ...
        fromRadioScratch.mqttClientProxyMessage =
            *mqttClientProxyMessageForPhone;
        releaseMqttClientProxyPhonePacket();
      }
    } else if (xmodemPacketForPhone.control != meshtastic_XModem_Control_NUL) {
#ifdef MESHTASTIC_PHONEAPI_ACCESS_CONTROL
      if (!getAdminAuthorized()) {
        xmodemPacketForPhone =
            meshtastic_XModem_init_zero; // Discard - unauthorized client
      } else
#endif
      {
        fromRadioScratch.which_payload_variant =
      ...
      if (auto *slot = findOrAllocStatusSlot_LH(this);
          slot && slot->hasPending) {
        fromRadioScratch.which_payload_variant =
            meshtastic_FromRadio_lockdown_status_tag;
...
void PhoneAPI::sendConfigComplete() {
  ...
  const bool shouldReplaySatellites =
  ...
  replayPhase =
      shouldReplaySatellites ? REPLAY_PHASE_POSITIONS : REPLAY_PHASE_IDLE;
  ...
  if (!EncryptedStorage::isLockdownActive()) {
    ...
    if (!EncryptedStorage::isProvisioned()) {
      queueLockdownStatus(meshtastic_LockdownStatus_State_NEEDS_PROVISION, "",
                          0, 0, 0);
      LOG_INFO("PhoneAPI: NEEDS_PROVISION sent to client");
    } else if (!EncryptedStorage::isUnlocked()) {
      queueLockdownStatus(meshtastic_LockdownStatus_State_LOCKED,
                          EncryptedStorage::getLockReason(), 0, 0, 0);
      LOG_INFO("PhoneAPI: LOCKED (%s) sent to client",
               EncryptedStorage::getLockReason());
    } else {
      queueLockdownStatus(meshtastic_LockdownStatus_State_LOCKED, "needs_auth",
                          0, 0, 0);
...
void PhoneAPI::prefetchNodeInfos() {
  ...
  // responsive even if DB fetches take a moment.
  {
    ...
    while (nodeInfoQueue.size() < kNodePrefetchDepth) {
      ...
      info.last_heard =
          isUs ? getValidTime(RTCQualityFromNet) : info.last_heard;
...
void setReplayHopFields(meshtastic_MeshPacket &pkt,
                        const meshtastic_NodeInfoLite *header) {
  uint8_t hopLimit =
      Default::getConfiguredOrDefaultHopLimit(config.lora.hop_limit);
...
PhoneAPI::makeReplayPositionPacket(NodeNum num,
  ...
  // history/dedup.
  pkt.id =
      makeReplayPacketId(num, pkt.rx_time, meshtastic_PortNum_POSITION_APP);
  pkt.channel = 0;
  ...
  size_t len = pb_encode_to_bytes(pkt.decoded.payload.bytes,
                                  sizeof(pkt.decoded.payload.bytes),
                                  &meshtastic_Position_msg, &fullPos);
  pkt.decoded.payload.size = (pb_size_t)len;
  return pkt;
}

meshtastic_MeshPacket
PhoneAPI::makeReplayTelemetryPacket(NodeNum num,
  ...
  pkt.id = makeReplayPacketId(num, pkt.rx_time,
  ...
  size_t len = pb_encode_to_bytes(pkt.decoded.payload.bytes,
                                  sizeof(pkt.decoded.payload.bytes),
                                  &meshtastic_Telemetry_msg, &fullTel);
  pkt.decoded.payload.size = (pb_size_t)len;
  return pkt;
}
...
void PhoneAPI::beginReplayPositions() {
  ...
  replayPositionIndex = 0;
  LOG_INFO("Begin position replay: %u entries millis=%u",
           (unsigned)replayPositionOrder.size(), millis());
#endif
}
...
void PhoneAPI::prefetchReplayPositions() {
  ...
  bool wasEmpty = false;
  {
    ...
    while (replayQueue.size() < kReplayPrefetchDepth &&
           replayPositionIndex < replayPositionOrder.size()) {
...
void PhoneAPI::beginReplayTelemetry() {
  ...
  replayTelemetryIndex = 0;
#else
  replayTelemetryOrder =
      nodeDB->snapshotTelemetryNodeNums(nodeDB->getNodeNum());
  replayTelemetryIndex = 0;
  LOG_INFO("Begin telemetry replay: %u entries millis=%u",
           (unsigned)replayTelemetryOrder.size(), millis());
#endif
}
...
void PhoneAPI::prefetchReplayTelemetry() {
  ...
  bool wasEmpty = false;
  {
    ...
    wasEmpty = replayQueue.empty();
    while (replayQueue.size() < kReplayPrefetchDepth &&
           replayTelemetryIndex < replayTelemetryOrder.size()) {
...
meshtastic_MeshPacket PhoneAPI::makeReplayEnvironmentPacket(
    uint32_t num, const meshtastic_EnvironmentMetrics &env) {
  meshtastic_MeshPacket pkt = meshtastic_MeshPacket_init_default;
  pkt.from = num;
  pkt.to = NODENUM_BROADCAST;
  const meshtastic_NodeInfoLite *header = nodeDB->getMeshNode(num);
...
void PhoneAPI::beginReplayEnvironment() {
  ...
  replayEnvironmentIndex = 0;
#else
  replayEnvironmentOrder =
      nodeDB->snapshotEnvironmentNodeNums(nodeDB->getNodeNum());
  replayEnvironmentIndex = 0;
  LOG_INFO("Begin environment replay: %u entries millis=%u",
           (unsigned)replayEnvironmentOrder.size(), millis());
#endif
}
...
void PhoneAPI::prefetchReplayEnvironment() {
  ...
  bool wasEmpty = false;
  {
    ...
    wasEmpty = replayQueue.empty();
    while (replayQueue.size() < kReplayPrefetchDepth &&
           replayEnvironmentIndex < replayEnvironmentOrder.size()) {
...
meshtastic_MeshPacket
PhoneAPI::makeReplayStatusPacket(uint32_t num,
  ...
  pkt.rx_time = header ? header->last_heard : 0;
  pkt.id =
      makeReplayPacketId(num, pkt.rx_time, meshtastic_PortNum_NODE_STATUS_APP);
  pkt.channel = 0;
  pkt.rx_snr = header ? header->snr : 0;
  setReplayHopFields(pkt, header);
  pkt.priority = meshtastic_MeshPacket_Priority_BACKGROUND;
  ...
  size_t len = pb_encode_to_bytes(pkt.decoded.payload.bytes,
                                  sizeof(pkt.decoded.payload.bytes),
                                  &meshtastic_StatusMessage_msg, &status);
  pkt.decoded.payload.size = (pb_size_t)len;
  return pkt;
}
...
void PhoneAPI::beginReplayStatus() {
  ...
  replayStatusIndex = 0;
  LOG_INFO("Begin status replay: %u entries millis=%u",
           (unsigned)replayStatusOrder.size(), millis());
#endif
}
...
void PhoneAPI::prefetchReplayStatus() {
  ...
  bool wasEmpty = false;
  {
    ...
    wasEmpty = replayQueue.empty();
    while (replayQueue.size() < kReplayPrefetchDepth &&
           replayStatusIndex < replayStatusOrder.size()) {
...
void PhoneAPI::advanceReplayPhase() {
  switch (replayPhase) {
  case REPLAY_PHASE_POSITIONS:
    LOG_DEBUG("Replay drain: positions done (count=%u) millis=%u",
              (unsigned)replayPositionIndex, millis());
    replayPhase = REPLAY_PHASE_TELEMETRY;
    break;
  case REPLAY_PHASE_TELEMETRY:
    LOG_DEBUG("Replay drain: telemetry done (count=%u) millis=%u",
              (unsigned)replayTelemetryIndex, millis());
    replayPhase = REPLAY_PHASE_ENVIRONMENT;
    break;
  case REPLAY_PHASE_ENVIRONMENT:
    LOG_DEBUG("Replay drain: environment done (count=%u) millis=%u",
              (unsigned)replayEnvironmentIndex, millis());
    replayPhase = REPLAY_PHASE_STATUS;
    break;
  case REPLAY_PHASE_STATUS:
    LOG_INFO("Replay drain complete (status count=%u) millis=%u",
             (unsigned)replayStatusIndex, millis());
    replayPositionOrder.clear();
...
void PhoneAPI::releaseMqttClientProxyPhonePacket() {
  if (mqttClientProxyMessageForPhone) {
    service->releaseMqttClientProxyMessageToPool(
        mqttClientProxyMessageForPhone);
...
void PhoneAPI::sendNotification(meshtastic_LogRecord_Level level,
                                uint32_t replyId, const char *message) {
...
bool PhoneAPI::wasSeenRecently(uint32_t id) {
  ...
  // the end
  memmove(recentToRadioPacketIds, recentToRadioPacketIds + 1,
          (19) * sizeof(uint32_t));
  recentToRadioPacketIds[19] = id;
  return false;
}

PhoneAPI::LocalAdminGate
PhoneAPI::classifyLocalAdminPacket(const meshtastic_MeshPacket &p,
...
bool PhoneAPI::handleToRadioPacket(meshtastic_MeshPacket &p) {
  ...
  // passphrase, so bound its lifetime.
  {
    ...
    case LocalAdminGate::LockdownAuth: {
      ...
      for (size_t i = 0; i < sizeof(admin.lockdown_auth.passphrase.bytes); i++)
        adminVol[i] = 0;
      volatile uint8_t *encodedVol =
          const_cast<volatile uint8_t *>(p.decoded.payload.bytes);
      for (size_t i = 0; i < sizeof(p.decoded.payload.bytes); i++)
        encodedVol[i] = 0;
      p.decoded.payload.size =
          0; // keep the length consistent with the wiped buffer
      return true;
    }
  ...
  if (p.decoded.portnum == meshtastic_PortNum_TRACEROUTE_APP &&
      ...
      Throttle::isWithinTimespanMs(lastPortNumToRadio[p.decoded.portnum],
                                   THIRTY_SECONDS_MS)) {
    LOG_WARN("Rate limit portnum %d", p.decoded.portnum);
    sendNotification(meshtastic_LogRecord_Level_WARNING, p.id,
                     "TraceRoute can only be sent once every 30 seconds");
    meshtastic_QueueStatus qs = router->getQueueStatus();
    service->sendQueueStatusToPhone(qs, 0, p.id);
    return false;
  } else if (p.decoded.portnum == meshtastic_PortNum_TRACEROUTE_APP &&
             isBroadcast(p.to) && p.hop_limit > 0) {
    sendNotification(
        meshtastic_LogRecord_Level_WARNING, p.id,
        "Multi-hop traceroute to broadcast address is not allowed");
    meshtastic_QueueStatus qs = router->getQueueStatus();
    service->sendQueueStatusToPhone(qs, 0, p.id);
    return false;
  } else if (IS_ONE_OF(p.decoded.portnum, meshtastic_PortNum_POSITION_APP,
    ...
    meshtastic_QueueStatus qs = router->getQueueStatus();
    service->sendQueueStatusToPhone(qs, 0, p.id);
    service->sendRoutingErrorResponse(
        meshtastic_Routing_Error_RATE_LIMIT_EXCEEDED, &p);
...
int PhoneAPI::onNotify(uint32_t newValue) {
  bool timeout =
      checkConnectionTimeout(); // a handy place to check if we've heard from
...
void PhoneAPI::completePendingUnlocks(bool reloadOk) {
  ...
  if (reloadOk) {
    ...
    for (size_t i = 0; i < targetCount; i++) {
      ...
      p->queueLockdownStatus(meshtastic_LockdownStatus_State_UNLOCKED, "",
                             boots, until, 0);
    }
    ...
    LOG_INFO("Lockdown: post-reload completion: authorized %u connection(s)",
    ...
    for (size_t i = 0; i < targetCount; i++) {
      targets[i]->queueLockdownStatus(meshtastic_LockdownStatus_State_LOCKED,
                                      "storage_corrupt", 0, 0, 0);
    }
...
void PhoneAPI::queueLockdownStatus(meshtastic_LockdownStatus_State state,
                                   ...
                                   uint32_t backoff_seconds) {
  {
    ...
    buildStatus_LH(slot->status, state, lock_reason, boots_remaining,
                   valid_until_epoch, backoff_seconds);
...
void PhoneAPI::broadcastLockdownStatus(meshtastic_LockdownStatus_State state,
                                       const char *lock_reason,
                                       uint8_t boots_remaining,
                                       uint32_t valid_until_epoch,
                                       uint32_t backoff_seconds) {
  bool anyOverwritten = false;
  {
    ...
    for (auto &s : g_statusSlots) {
      if (s.who) {
        buildStatus_LH(s.status, state, lock_reason, boots_remaining,
                       valid_until_epoch, backoff_seconds);
        s.hasPending = true;
        anyOverwritten = true;
      }
...
bool PhoneAPI::handleLockdownAuthInline(const meshtastic_LockdownAuth &la) {
  ...
  auto zeroPassphrase = [&]() {
    volatile uint8_t *ppVol =
        const_cast<volatile uint8_t *>(la.passphrase.bytes);
  ...
  if (la.lock_now) {
    if (!getAdminAuthorized()) {
      ...
      queueLockdownStatus(meshtastic_LockdownStatus_State_UNLOCK_FAILED, "", 0,
                          0, 0);
      zeroPassphrase();
      return true;
    }
  ...
  if (la.disable) {
    ...
    if (!EncryptedStorage::isLockdownActive()) {
      ...
      queueLockdownStatus(meshtastic_LockdownStatus_State_DISABLED, "", 0, 0,
                          0);
      zeroPassphrase();
      return true;
    }
    // Re-verify the passphrase (loads the DEK needed to decrypt files).
    bool ok = EncryptedStorage::unlockWithPassphrase(
        la.passphrase.bytes, la.passphrase.size,
    ...
    lockdownDisablePending =
        true; // main loop runs nodeDB->disableLockdownToPlaintext() then
              // reboots
    LOG_INFO(
        "Lockdown: disable authorized, deferring decrypt-revert to main loop");
    zeroPassphrase();
    return true;
  }
  ...
  if (la.passphrase.size < 1) {
    LOG_WARN("Lockdown: lockdown_auth with empty passphrase and lock_now=false "
             "- rejecting");
    queueLockdownStatus(meshtastic_LockdownStatus_State_UNLOCK_FAILED, "", 0, 0,
                        0);
    zeroPassphrase();
    return true;
  }
  ...
  if (la.boots_remaining > 255) {
    LOG_WARN("Lockdown: boots_remaining=%u exceeds uint8 cap, rejecting",
             la.boots_remaining);
    queueLockdownStatus(meshtastic_LockdownStatus_State_UNLOCK_FAILED, "", 0, 0,
                        0);
    zeroPassphrase();
  ...
  uint8_t boots = la.boots_remaining != 0
                      ? (uint8_t)la.boots_remaining
  ...
  uint32_t sessionMaxSeconds =
      la.max_session_seconds != 0 ? la.max_session_seconds
                                  : MESHTASTIC_LOCKDOWN_SESSION_DEFAULT_SECONDS;
  ...
  if (!EncryptedStorage::isUnlocked()) {
    if (!EncryptedStorage::isProvisioned()) {
      ...
      ok = EncryptedStorage::provisionPassphrase(
          la.passphrase.bytes, la.passphrase.size, boots, validUntilEpoch,
          sessionMaxSeconds);
    } else {
      LOG_INFO("Lockdown: unlock with passphrase");
      ok = EncryptedStorage::unlockWithPassphrase(
          la.passphrase.bytes, la.passphrase.size, boots, validUntilEpoch,
          sessionMaxSeconds);
    }
    if (ok) {
