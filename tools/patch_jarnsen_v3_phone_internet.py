"""Keep phone Internet available beside the V3 local service WLAN and auto-check firmware.

This patch runs after patch_jarnsen_v3_service_portal.py.  It deliberately keeps
...
for GitHub/map traffic while reaching the V3 locally at 192.168.4.1.
"""
...
# Expose the build SHA stamped by the existing V3 workflow and use the official
# ESP-NETIF DHCP API for a local-only AP (DHCP option 3/router disabled).
include_anchor = '#include "mesh/http/JarnsenV3ServicePortalPage.h"\n'
include_new = include_anchor + '#include "infrastructure/HeltecV3BuildInfo.h"\n'
web = once(web, include_anchor, include_new, "V3 build-info include")
...
status_tail = r'''        "\"track_count\":%u,\"long_name\":\"%s\",\"short_name\":\"%s\",\"node_id\":\"!%08x\",\"role\":\"%s\"}",
...
'''
status_tail_new = r'''        "\"track_count\":%u,\"long_name\":\"%s\",\"short_name\":\"%s\",\"node_id\":\"!%08x\",\"role\":\"%s\",\"build_sha\":\"%s\"}",
        DEVICE_TITLE, DEVICE_CODE, serviceSsid, sessionToken, GITHUB_TAG, FIRMWARE_ASSET,
        (unsigned)jarnsenPositionTrackCount(), longName, shortName, (unsigned)nodeNum, serviceRoleName(), JARNSEN_V3_BUILD_SHA);
'''
web = once(web, status_tail, status_tail_new, "build SHA in service status")
...
portal = once(
    ...
    '<small id="barState">V3 lokal · Internet über Mobilfunk</small>',
    "portal network banner",
)
portal = once(
    portal,
    '<div id="overviewStatus" class="status">Lokale Servicedaten bereit.</div>',
    '<div id="overviewStatus" class="status">V3 lokal über WLAN · Internet/Online-Karten über Mobilfunk.</div>',
...
async function checkFirmware(){const btn=$('githubBtn');btn.disabled=true;status('fwStatus','GitHub-Firmwarestand wird geprüft …');$('fwState').textContent='Prüfe …';try{const a=await latest(),installed=String(info?.build_sha||'').toLowerCase(),remote=String(a.sourceSha||'').toLowerCase(),known=installed&&installed!=='unknown',current=known&&remote&&remote.startsWith(installed);$('fwInstalled').textContent=known?installed:'unbekannt';$('fwLatest').textContent=remote?remote.slice(0,8):'verfügbar';if(current){$('mFw').textContent=installed+' · aktuell';$('fwState').textContent='Aktuell ✓';btn.textContent='Firmware aktuell ✓';btn.disabled=true;status('fwStatus','Firmware ist aktuell · Build '+installed,'ok')}else{$('mFw').textContent=known?installed+' · Update verfügbar':'Update verfügbar';$('fwState').textContent='Update verfügbar';btn.textContent='Update installieren';btn.disabled=false;status('fwStatus',remote?'Update verfügbar · installiert '+(known?installed:'unbekannt')+' · GitHub '+remote.slice(0,8):'GitHub-Firmware verfügbar; Buildvergleich nicht möglich.','warn')}}catch(e){const installed=String(info?.build_sha||'');$('fwInstalled').textContent=installed||'–';$('fwLatest').textContent='offline';$('fwState').textContent='Keine Prüfung';$('mFw').textContent=(installed||'–')+' · nicht geprüft';btn.textContent='Firmware erneut prüfen';btn.disabled=false;status('fwStatus','GitHub-Prüfung nicht möglich – Mobilfunk/Internet prüfen: '+(e.name==='AbortError'?'Zeitüberschreitung':e.message),'warn')}}
'''
portal = replace_between(portal, "async function latest(){", "function hex(buf)", latest_and_check, "latest firmware and automatic comparison")

upload_selected_old = "async function uploadSelected(){try{const f=$('file').files[0];if(!f)throw Error('Keine Firmware gewählt');const a=asset?{sha:''}:await latest();await postFirmware(f,a.sha||'')}catch(e){status('fwStatus','Update fehlgeschlagen: '+e.message,'err')}}"
upload_selected_new = "async function uploadSelected(){try{const f=$('file').files[0];if(!f)throw Error('Keine Firmware gewählt');const a=latestMeta||await latest();await postFirmware(f,a.sha||'')}catch(e){status('fwStatus','Update fehlgeschlagen: '+e.message,'err')}}"
portal = once(portal, upload_selected_old, upload_selected_new, "manual upload GitHub hash verification")
...
for marker in (
    ...
    if marker not in web:
        raise SystemExit(f"missing V3 phone-Internet web marker: {marker}")
...
for marker in (
    ...
    if marker not in portal:
        raise SystemExit(f"missing V3 phone-Internet portal marker: {marker}")

WEB.write_text(web, encoding="utf-8")
PORTAL.write_text(portal, encoding="utf-8")
print("Heltec V3: local-only service WLAN, cellular Internet coexistence and automatic firmware check enabled")
