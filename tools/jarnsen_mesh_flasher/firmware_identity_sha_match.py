...
def install(services: Any) -> None:
    ...
    the JARNSEN image.  Correlating it with successful Unified-Core runs gives
    us an exact JARNSEN version/build without a second slow serial --info pass.
    """
    ...
    def query_jarnsen_identity(port: str, timeout: float = 1.8):
        ...
        return base_query(port, timeout=timeout)
    ...
    _emit(
        ...
        "exact-version-build=1 slow-second-info-avoided=1"
    )
