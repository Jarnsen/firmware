"""v2.1.11: make Meshtastic config BLE safe in PyInstaller --windowed builds."""
...
def patch(source: str) -> str:
    ...
    source = source.replace('APP_VERSION != "2.1.9"', 'APP_VERSION != "2.1.11"')
    source = source.replace("App-Version ist nicht v2.1.9", "App-Version ist nicht v2.1.11")
...
'''
    ...
    if missing:
        raise SystemExit("v2.1.11 validation failed: " + ", ".join(missing))
    return source


def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: patch_jarnsen_service_tool_v2111.py <source.py>")
