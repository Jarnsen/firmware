...
try{
 ...
 const capture=async(name,selector)=>{await page.waitForSelector(selector,{timeout:8000});await page.screenshot({path:path.join(out,`v400-${name}-1920x1080-125pct.png`),fullPage:true});};
 await capture('01-dashboard','.neo-dashboard');
 await page.locator('[data-neo-page="nodes"]').click();await capture('02-nodes','.neo-nodes');
 await page.locator('.v323-node-row').first().locator('button[data-action="inspect"]').click();await capture('03-node-details','.neo-details');
 for(const [pageId,name,selector] of [['logs','04-logs','.neo-logs-grid'],['firmware','05-firmware','.neo-firmware-grid'],['power','06-power','.rd-power-page'],['profiles','07-profiles','.neo-profiles'],['network','08-network','.rd-network-page'],['display','09-display','.neo-display-grid'],['tools','10-tools','.neo-tools-grid'],['settings','11-settings','.neo-settings-grid']]){await page.locator(`[data-neo-page="${pageId}"]`).click();await capture(name,selector)}
 ...
 fs.writeFileSync(path.join(out,'v400-page-reference-summary.json'),JSON.stringify({ok:true,physical:'1920x1080',windowsScalePercent:125,effectiveCssViewport:'1536x864',pages:shots},null,2));
 console.log(`Captured ${shots.length} real v4 page references.`);
