...
function findBrowser() {
  ...
  if (!found) throw new Error('Chrome/Edge not found for 125% scale test');
  return found;
}
...
console.log(`[125pct] server-ready ${port}`);
let browser;
try {
  const executablePath = findBrowser();
  console.log(`[125pct] browser ${executablePath}`);
  ...
  console.log('[125pct] goto');
  await page.goto(url, {waitUntil:'domcontentloaded', timeout:15000});
  await sleep(2500);
  assert(await page.locator('body[data-neo-ui="v400"]').count() === 1, '125%: Neo UI v4 marker missing');
  assert(await page.locator('.neo-dashboard').count() === 1, '125%: v4 dashboard missing');
  ...
  const assertLayout = async label => {
    const data = await page.evaluate(() => {
      ...
      return {innerWidth,innerHeight,scrollWidth:document.documentElement.scrollWidth,sidebar:rect('.sidebar'),main:rect('.main-column'),inspector:rect('.inspector'),top:rect('#v323TopStrip'),content:rect('.content-card'),shell:rect('.rd-v323-shell')};
    });
  ...
  await sleep(250);
  await page.screenshot({path:path.join(outDir,'usb-log-prompt-1920x1080-125pct.png'),fullPage:false,animations:'disabled',caret:'hide',timeout:10000});
  await prompt.getByRole('button', {name:'Nicht herunterladen', exact:true}).click();
  await sleep(300);
  assert(await prompt.count() === 0, '125%: USB prompt did not close after decline');
  ...
  console.log('[125pct] dashboard');
  await assertLayout('Dashboard');
  assert(await page.locator('.neo-dashboard .v323-metric').count() === 4, 'Dashboard KPI layout incomplete at 125% scaling');
  assert(await page.locator('.neo-dashboard .v323-quick-grid button').count() >= 6, 'Dashboard quick actions incomplete at 125% scaling');
  await page.screenshot({path:path.join(outDir,'dashboard-1920x1080-125pct.png'),fullPage:false,animations:'disabled',caret:'hide',timeout:10000});

  console.log('[125pct] nodes');
  await page.locator('.nav-item[data-neo-page="nodes"]').click();
  await sleep(500);
  assert(await page.locator('.neo-nodes').count() === 1, '125%: v4 Nodes renderer missing');
  await assertLayout('Nodes');
  assert(await page.locator('.neo-node-table.v323-node-row').count() === 4, 'Nodes rows missing at 125% scaling');
  ...
  await page.locator('.neo-node-table.v323-node-row').first().locator('button[data-action="inspect"]').first().click();
  await sleep(250);
  assert((await page.locator('.inspector').innerText()).includes('RiKrTrp MrsZg26'), 'Inspector click path failed at 125% scaling');
  await page.screenshot({path:path.join(outDir,'nodes-1920x1080-125pct.png'),fullPage:false,animations:'disabled',caret:'hide',timeout:10000});

  assert(errors.length === 0, `125% scale browser errors:\n${errors.join('\n')}`);
  fs.writeFileSync(path.join(outDir,'ui-check-1920x1080-125pct.json'), JSON.stringify({ok:true,physical:'1920x1080',windowsScalePercent:125,effectiveCssViewport:'1536x864',deviceScaleFactor:SCALE,usbPrompt:'visible-and-decline-tested',browser:executablePath}, null, 2));
  console.log('1920x1080 @ 125% UI validation OK.');
} finally {
  console.log('[125pct] cleanup');
  await closeBounded(browser);
  server.closeIdleConnections?.(); server.closeAllConnections?.(); server.close();
}
